...
def exp_w1p(x,x0):
    return alpha*(1/(1+a21*np.exp(-1j*(x-x0)))).real
...
for i in range(len(ps_pos)):
    P0=[(np.amax(matrix[i])+np.amin(matrix[i]))/2, np.amax(matrix[i])-np.amin(matrix[i]), 0.1,1.8]
    print(P0)
    B0=([0,0,0.1, 1.5],[2500,2500,0.15,2])
