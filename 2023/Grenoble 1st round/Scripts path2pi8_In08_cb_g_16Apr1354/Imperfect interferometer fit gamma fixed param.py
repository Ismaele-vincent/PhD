...
def fit_I_px(x, gamma0, chi0, w_c, A, B):
    ...
    for i in range(len(chi)):
        if i==0:
            fit_I_px = A*(I_px_co(beta[i], chi[i], C, alpha, gamma) + I_px_in(beta[i], chi[i], eta, alpha, gamma)) + B*0
        else:
            fit_I_px = np.vstack((fit_I_px, A*(I_px_co(beta[i], chi[i], C, alpha, gamma) + I_px_in(beta[i], chi[i], eta, alpha, gamma)) + B*0))
    # print(fit_I_px)
    return fit_I_px.ravel()
