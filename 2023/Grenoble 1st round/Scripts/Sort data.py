...
for root, dirs, files in os.walk(fold_path, topdown=False):
    ...
    for name in files:
        """
        Beta and gamma scan sorting
        """
        # if (name in inf_files[:,0]):
        #     if name[:5] == inf_file_name[:5]:
        #         shutil.copy(os.path.join(root, name), beta_fold_raw+"/"+name)
        ...
        #         with open(beta_fold_clean+"/beta_ps_"+str("%02d" % (ps_num[0][ps_num[1]==ps][0],)[0])+".txt", 'w') as f:
