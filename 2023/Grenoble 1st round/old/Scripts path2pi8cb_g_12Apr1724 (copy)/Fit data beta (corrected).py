...
for i in range(len(ps_pos)):
    ...
    p,cov=fit(fit_cos,c_pos,matrix[i], p0=P0)#, sigma=matrix_err[i])
    err=np.diag(cov)**0.5
    # print(p[3], err[3])
...
ax.errorbar((ps_pos-ps_pos[0]-p1[-1])*w_ps/np.pi, beta/alpha, yerr=err_b/alpha,fmt="ko",capsize=5)
# ax.plot((x_plt-x_plt[0]-p1[-1])*w_ps/np.pi,fit_w1p(x_plt, *p1),"b", label="Fit Re{"+"$\omega_{1+}$}")
ax.plot((x_plt-x_plt[0]-p1[-1])*w_ps/np.pi,exp_w1p(x_plt, p1[-1])/alpha,"g", label="Exp Re{"+"$\omega_{1+}$}")
# ax.set_ylim([10,50])
