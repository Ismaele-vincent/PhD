...
def fit_w1p(x,A,th,x0):
    return alpha*(1-A+(1/(1+a21*np.tan(th*np.pi/4)**2*np.exp(-1j*(w_ps*(x-x0)))))).real
...
fit_res0=[1.82899076e+02,  1.65419666e+02,  4.88948118e+01, -1.25389305e-02]
err_res0=[3.60048888e-01, 4.87868336e-01, 4.68491347e-02, 6.19206548e-05]
max0=fit_res0[-1]
for i in range(len(ps_pos)):
    matrix[i]=tot_data[:,2][tot_data[:,-1]==ps_pos[i]]/tot_data[:,1][tot_data[:,-1]==ps_pos[i]]
for i in range(len(ps_pos)):
    P0=[500, 500, 48,-0.0125]
    # print(P0)
    ...
    ax.errorbar(c_pos,matrix[i],yerr=np.sqrt(matrix[i])/10,fmt="ko",capsize=5)
...
ax[0].errorbar((ps_pos-ps_pos[0]-p1[-1])*w_ps/np.pi,-beta, yerr=err_b/alpha*0,fmt="ko",capsize=5)
