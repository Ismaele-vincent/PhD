...
B0=([c_0-5, ps_0-5, w_c-5, w_ps-5,0,0],[c_0+5,ps_0+5, w_c+5, w_ps+5,np.inf,np.inf])
...
def I_px_new(gamma0, chi0, w_c, w_ps, C, eta):
    ...
    chi = w_ps*ps_pos-chi0
    gamma, chi = np.meshgrid(gamma, chi)
    fit_I_px = 2*I_px_co(np.pi, chi, C, alpha, gamma)
    # print(fit_I_px)
    return fit_I_px
...
corrected_matrix=np.array(Z2)#I_px_new(*p)*np.amax(matrix)
