...
for inf_file_name in inf_file_names:
    ...
    for i in range(len(ps_pos)):
        ...
        c_0_data=abs(yf_data[abs(xf)<1/S_F/2]).astype(complex)-34242.28911384991
        var=np.sum(matrix_err)**0.5/2
        c_1_data_err=var
        c_0_data_err=(var**2+ 2026.169279838217**2)**0.5
    ...
    ax.errorbar(chi, Im_data, Im_data_err, fmt="k.", capsize=5)
