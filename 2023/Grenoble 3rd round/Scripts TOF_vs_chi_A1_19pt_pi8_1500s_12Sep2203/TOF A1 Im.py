...
chi=0
chi_0=0
Co=0.6526851007101038
Co_err=0.020031617082422556
a_21=1
...
for inf_file_name in inf_file_names:
    ...
    for i in range(len(ps_pos)):
        ...
        c_0_data=abs(yf_data[abs(xf)<1/S_F/2]).astype(complex)-19701.287083776213
        var=np.sum(matrix_err)**0.5/2
        c_1_data_err=var
        c_0_data_err=(var**2+ 1868.9214713980712**2)**0.5
    ...
    ax.plot(chi_plt, w1(chi_plt).imag,"b-", label="Im($w_{1,+}$) theory")
    ax.errorbar(chi, Im_data, Im_data_err, fmt="o", capsize=5, color=(0.1,0.1,0.6), label="Im($w_{1,+}$) data")
    ax1.plot(chi_plt, w1(chi_plt).real,"r-", label="Re($w_{1,+}$) theory")
    ...
    if a_21==1:
        ax.set_ylim([-5,5])  
    # plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 3rd round/Report/Images/Im A1 corrected pi8 balanced.pdf", format="pdf", bbox_inches="tight")
