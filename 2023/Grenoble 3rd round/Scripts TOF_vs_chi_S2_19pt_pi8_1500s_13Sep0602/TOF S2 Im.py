...
for inf_file_name in inf_file_names:
    ...
    for i in range(len(ps_pos)):
        ...
        c_0_data=abs(yf_data[abs(xf)<1/S_F/2]).astype(complex)-11282.782542911855#-22117.395363815085#-21894.845382878237
        var=np.sum(func_data)**0.5/2
        c_1_data_err=var
        c_0_data_err=var+2108.147055646059#+1460.624019847072
    ...
    B0=([1000,1,-10],[10000000,4,10])
    p_cos_unb,cov_unb=fit(fit_cos_unb, ps_pos, cos2, p0=[250000,3,0], bounds=B0)
