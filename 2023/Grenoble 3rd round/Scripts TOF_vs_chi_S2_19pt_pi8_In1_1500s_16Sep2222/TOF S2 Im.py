...
if (inf_file_names[0][21])=="1":
    a=16
alpha_2=0.3123806231229692#0.37#np.pi/a #/2.354
...
for inf_file_name in inf_file_names:
    ...
    for i in range(len(ps_pos)):
        ...
        c_0_data=abs(yf_data[abs(xf)<1/S_F/2]).astype(complex)-22117.395363815085#-21894.845382878237
        var=np.sum(func_data)**0.5/2
        c_1_data_err=var
        c_0_data_err=var+1460.624019847072
    ...
    # ax.plot(chi+np.pi, Re_chi_pi,"ro")
    ax.legend()
    # ax.set_ylim([-1,2.1])  
    ...
    datatxt= np.array([chi,Im_data, Im_data_err,])
    with open(inf_file_name[:8]+"_Im_corrected.txt","w") as f:
