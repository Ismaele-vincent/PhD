# -*- coding: utf-8 -*-
"""
Created on Sun Aug 27 15:01:28 2023

@author: S18
"""
"""
bad apples:
"ifg_5s_1p_24Aug1419",
"ifg_5s_1p_25Aug1016",
"ifg_5s_1p_24Aug1806",
"ifg_5s_1p_24Aug1522",
"ifg_5s_1p_24Aug1519",
"ifg_5s_1p_27Aug1602",
"ifg_5s_1p_28Aug1554", 
"ifg_20s_3p_25Aug1739",
"ifg_20s_3p_25Aug1404",
"ifg_20s_3p_28Aug1007",
"movePS_90_01Sep1538",
"movePS_90_01Sep1529", 
"ifg_TOF_S2_09Sep1626", 
"ifg_TOF_A1_07Sep1405", 
"ifg_TOF_S2_10Sep1828", 
"""

import os
import numpy as np
import shutil

sc_fold_path="/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 3rd round/exp_3-16-14/rawdata/sc/"

bad_apples=[
"ifg_5s_1p_24Aug1419",
"ifg_5s_1p_25Aug1016",
"ifg_5s_1p_24Aug1806",
"ifg_5s_1p_24Aug1522",
"ifg_5s_1p_24Aug1519",
"ifg_5s_1p_27Aug1602",
"ifg_5s_1p_28Aug1554",
"ifg_20s_3p_25Aug1739",
"ifg_20s_3p_25Aug1404",
"ifg_20s_3p_28Aug1007",
"movePS_90_01Sep1538",
"movePS_90_01Sep1529", 
"ifg_TOF_S2_09Sep1626", 
"ifg_TOF_A1_07Sep1405", 
"ifg_TOF_S2_10Sep1828", ]
for root, dirs, files in os.walk(sc_fold_path, topdown=False):
    for name in files:
        if ("ifg_TOF_A1_pi8_26Sep1107" in name):#(("ifg_" in name) or ("movePS" in name)) and (".inf" in name) and ("TOF_vs_chi" not in name) and ("alpha" not in name) :
            if (name[:-4] in bad_apples):
                print('bad "'+name[:-4]+'", ')
            else:
                print('"'+name[:-4]+'", ')
                inf_file_path=os.path.join(root, name)
                inf_file_name=name[:-4]
                sorted_fold_path="/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 3rd round/exp_3-16-14/Sorted data/Ifg/"+inf_file_name
                
                if not os.path.exists(sorted_fold_path):
                    os.makedirs(sorted_fold_path)
    
                rawdata=sorted_fold_path+"/Rawdata" 
                if not os.path.exists(rawdata):
                    os.makedirs(rawdata)
    
                cleandata=sorted_fold_path+"/Cleantxt"
                if not os.path.exists(cleandata):
                    os.makedirs(cleandata)
                    
    
                dat_files = np.genfromtxt(inf_file_path, dtype=str, usecols=0)
                for root, dirs, files in os.walk(sc_fold_path, topdown=False):
                    i=0
                    for name in files:
                        if (name in dat_files):
                            # print(name)
                            shutil.copy(os.path.join(root, name), rawdata+"/"+name)
                            if "movePS" in name:
                                cleantxt=np.loadtxt(os.path.join(root, name), encoding='windows-1252', comments="*", delimiter="\t")[:,1:]
                            else:
                                cleantxt=np.loadtxt(os.path.join(root, name), encoding='windows-1252', comments="*", delimiter="\t")[:,1:]
                            with open(cleandata+"/"+name[:-4]+".txt", 'w') as f:
                                    np.savetxt(f, cleantxt, header= " ps_pos exposure_time(s) O-Beam H-Beam Monitor AUX-Beam time(s) O+H+AUX encod1 encod2", fmt='%f %.1f %i %i %i %i %i %i %f %f')