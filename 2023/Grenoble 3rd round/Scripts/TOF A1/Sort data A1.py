...
"""
bad apples:
 "TOF_vs_chi_A1_27Aug1617",
 "TOF_vs_chi_A1_ifg_30Aug0652",
"""
import os
import numpy as np
import shutil
...
for root, dirs, files in os.walk(sc_fold_path, topdown=False):
    for name1 in files:
        if ("TOF_vs_chi_A1_" in name1) and (".inf" in name1) and ("CdS2" not in name1):
                ...
                for root, dirs, files in os.walk(sc_fold_path, topdown=False):
                    ...
                    for name in files:
                        if (name in dat_files[:,0]):
                            if ("ifg" in name1) and (name==dat_files[0,0] or name==dat_files[6,0] or name==dat_files[13,0]):
                                # print(name)
                                ...
                                func_gen=dat_files[:,-7:-4][dat_files[:,0]==name].astype(float)
                                # print(ps)
