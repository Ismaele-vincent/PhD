...
def fit_O_beam(t, A, C, chi, a_1, phi_1):
    # a_1=alpha(T,f_1,B_1)
...
for inf_file_name in inf_file_names:
    ...
    P0=[(np.amax(matrix[i])+np.amin(matrix[i]))/2, 0.6, 3*ps_pos[i],  alpha(T,f_1,B_1), 0.1]
    p_tot=np.zeros((len(ps_pos),len(P0)))
    err_tot=np.zeros((len(ps_pos),len(P0)))
    for i in range(len(ps_pos)):
        B0=([10,0.1,-3*np.pi,-3,-3*np.pi],[3000,0.8,3*np.pi, 0, 3*np.pi])
    ...
    param_names=["A", "C", "$\chi$", "-$\\alpha_1$", "$\phi_1$"]
    ...
    for i in range(len(param_names)):
        ...
        axs[i].errorbar(ps_pos, abs(p_tot[:,i]), yerr=err_tot[:,i])
