...
def fit_cos_unb(x, A, Co):
    return A*((1 - Co)/2 + Co*(1/2+a_1*a_2*np.cos(x)))
...
for inf_file_name in inf_file_names:
    ...
    for i in range(len(ps_pos)):
        ...
        c_1_data_err=var
        c_0_data_err=var+682#+558.4314672449891#
    ...
    B0=([0,0.1],[1000000,0.9])
    p_cos_unb,cov_unb=fit(fit_cos_unb, chi, cos2, p0=[100000, 0.75], bounds=B0)
    ...
    ax.plot(ps_plt, fit_cos_unb(chi_plt,*p_cos_unb), "b-", label="Fit")
    ax.legend()
    print("param cos_unb=",p_cos_unb)
    corr_unb=p_cos_unb[0]
    corr_err=(np.diag(cov_unb)**0.5)
    print("correction_unb="+str(p_cos_unb[0]*(1-p_cos_unb[-1])/2)+" +- "+str(0.5*((abs(1-p_cos_unb[-1])*corr_err[0])**2+(abs(p_cos_unb[0])*corr_err[-1])**2)**0.5))
    # if a_21==1:
    #     ax.set_ylim([-5,5])  
