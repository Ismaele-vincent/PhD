...
def fit_O_beam(t, A, C, chi0, a_2, xi_2):
    # a_2=alpha(T,f_2,B_2)
    # xi_2=phi_2+(2*np.pi*f_2*1e-3*T+np.pi)/2#-2*np.pi*f_2*1e3/v0
    chi_fit=chi-chi0
    return A+C*np.cos(chi_fit-a_2*np.sin(2*np.pi*1e-3*f_2*t+xi_2))

inf_file_names=["TOF_vs_chi_S2_26Aug1702"]
...
for inf_file_name in inf_file_names:
    ...
    P0=[np.average(matrix[i]), 500, chi0, alpha(T,f_2,B_2), -0.5]
    p_tot=np.zeros((len(ps_pos),len(P0)))
    err_tot=np.zeros((len(ps_pos),len(P0)))
    for i in range(len(ps_pos)):
        B0=([np.amin(matrix),10,-3*np.pi,-3,-3*np.pi],[np.amax(matrix),1500,3*np.pi, 0, 3*np.pi])
        chi=w_ps*ps_pos[i]
    ...
    param_names=["A", "C", "$\chi$", "$\\alpha_2$", "$\\xi_2$"]
    ...
    for i in range(len(param_names)):
        ...
        # if p_tot[:,i].all()<0:
        #     axs[i].set_ylim([np.amin(abs(p_tot[:,i]))*(0.9),np.amax(abs(p_tot[:,i]))*(1.1)])
