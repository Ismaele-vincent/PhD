# -*- coding: utf-8 -*-
"""
Created on Mon Sep  4 15:21:20 2023
...
chi=0
chi_0=0.7
C=0.6590# +- 0.02249113521097985
a=8
a_21=2
a_1=2/5**0.5
a_2=1/5**0.5
# inf_file_names=["TOF_vs_chi_S2_19pt_pi8_1500s_13Sep0602",]
# inf_file_names=["TOF_vs_chi_S2_19pt_pi16_1500s_14Sep0021",]
# inf_file_names=["TOF_vs_chi_S2_19pt_pi8_SD_1500s_15Sep1013",]
inf_file_names=["TOF_vs_chi_S2_19pt_pi8_In1_1500s_16Sep2222",]
# inf_file_names=["TOF_vs_chi_S2_19pt_pi8_In2_1500s_17Sep2350",]
# inf_file_names=["TOF_vs_chi_S2_19pt_pi4_1500s_26Sep1339",]
# In2, pi/8
# spin up, pi/4
# Un, pi/4
# Un, pi/8
...
if (inf_file_names[0][21])=="1":
    a=16
alpha_2=0.37#np.pi/a #/2.354
alpha_2_err=0.15*alpha_2
def w2(chi):
...
for inf_file_name in inf_file_names:
    ...
    for i in range(len(ps_pos)):
        ...
        c_0_data=abs(yf_data[abs(xf)<1/S_F/2]).astype(complex)-21894.845382878237
        var=np.sum(matrix_err)**0.5
        c_2_data_err=var
        c_1_data_err=var
        c_0_data_err=var+618#+558.4314672449891#
        ...
        # d=Im_data[i]**2 + 4*(c_2_data*e_m2xi).real/(alpha_2**2*cos2[i])
        d=Im_data[i]**2 + 2/alpha_2**2*(((c_0_data).real)/(cos2[i])-1)
        d_err = ((2*Im_data[i]*Im_data_err[i])**2 + (2/alpha_2**2*(1/cos2[i]*abs(c_0_data_err)))**2 + (2/alpha_2**2*(abs(c_0_data_err)/cos2[i]**2*abs(cos2_err[i])))**2)**0.5
        # print(chi[i], abs(d))
        Re_data[i]=(0.5+0.5*(1-4*d)**0.5)
        Re_data_err[i]=abs(2/abs(1-4*d)**0.5*d_err)
        # # print(np.amax(abs(yf))/N)
    p_cos2,cov=fit(fit_cos2, chi, cos2, p0=[10000, 200, 0])
    p_cos_unb,cov_unb=fit(fit_cos_unb, chi, cos2, p0=[5000, 0.1, 0])
    fig = plt.figure(figsize=(8,6))
