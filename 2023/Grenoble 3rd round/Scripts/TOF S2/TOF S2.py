...
def fit_O_beam(t, A, C, chi, B_2, phi_2, T):
    a_2=alpha(T,f_2,B_2)
    xi_2=phi_2+(2*np.pi*f_2*1e-3*T+np.pi)/2#-2*np.pi*f_2*1e3/v0
    return A*(1-C+C*(1+np.cos(chi-a_2*np.sin(2*np.pi*1e-3*f_2*t+xi_2)))/2)
...
for inf_file_name in inf_file_names:
    ...
    P0=[(np.amax(matrix[i])+np.amin(matrix[i]))/2, 0.6, 3*ps_pos[i], 10, 0, 10]
    p_tot=np.zeros((len(ps_pos),len(P0)))
    err_tot=np.zeros((len(ps_pos),len(P0)))
    for i in range(len(ps_pos)):
        B0=([np.amin(matrix),0.1,-3*np.pi,2,-3*np.pi,5],[np.amax(matrix),1,3*np.pi, 30, 3*np.pi,30])
