...
def fit_O_beam(t, A, C, chi, T, phi1):
    a1=alpha(T,f1,B1)
    xi1=phi1+(2*np.pi*f1*1e-3*T+np.pi)/2-2*np.pi*f1*1e3/v0
    return A+C*np.cos(chi-a1*np.sin(2*np.pi*1e-3*f1*t+xi1))/2

inf_file_names=["TOF_test_vs_chi_23Aug1857.inf", 
"TOF_test_vs_chi_24Aug0034.inf", 
"TOF_test_vs_chi_24Aug0612.inf", 
"TOF_test_vs_chi_24Aug1149.inf", 
"TOF_test_vs_chi_24Aug1822.inf", 
...
for inf_file_name in inf_file_names:
    ...
    P0=[(np.amax(matrix[i])+np.amin(matrix[i]))/2, np.amax(matrix[i])-np.amin(matrix[i]), 3*ps_pos[i], 10, 0.7]
    for i in range(len(ps_pos)):
        B0=([0,0.1,-3*np.pi,1,-3*np.pi],[2000,2000,3*np.pi, 30, 3*np.pi])
