...
data_fold_path="/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Results txt/No In"
...
ax.errorbar(ps_pos, cos2_avg,cos2_avg_err, fmt="k.")
...
fig = plt.figure(figsize=(8.5,5), dpi=150)
gs = fig.add_gridspec(2,3, height_ratios=(1,1),width_ratios=(1,1,1), hspace=0.0,wspace=0.25)
...
axsl[0].set_title("$w_{+,1}$", fontsize=11)#"a_2/a_1\\approx$"+str("%.2f" % (a_21),)+")")

axsl[0].errorbar(chi, Im_1_avg, Im_1_avg_err, fmt=".", color=colors[0], capsize=3, label="Data")
axsl[0].plot(chi_plt, w1(chi_plt).imag, "-",color=colors[3], alpha=0.8, label="Theory")
...
ylim_re_1=-1
ylim_re_2=5
y_re_labels=np.arange(ylim_re_1,ylim_re_2,1)

# axsl[1].tick_params(axis="x", bottom=False, labelbottom=False)
# axsl[1].set_title("$w_{+,2}$", fontsize=11)#"a_2/a_1\\approx$"+str("%.2f" % (a_21),)+")")
# axsl[1].errorbar(chi[:-7], Re_1_avg[:-7], Re_1_avg_err[:-7], fmt=".", color=colors[1], capsize=3, label="$\Im(w_{1,+})$ data")
axsl[1].errorbar(chi, Re_1_avg, Re_1_avg_err, fmt=".", color=colors[0], capsize=3)
axsl[1].plot(chi_plt, w1(chi_plt).real, "-", color=colors[3], alpha=0.8)
axsl[1].grid(True, ls="dotted")
# axsl[1].yaxis.set_label_coords(-0.2,0.5)
axsl[1].set_ylim([ylim_re_1,ylim_re_2])
# axsl[1].set_yticks(ticks=axsr[0].get_yticks()[1:-1])
axsl[1].set_yticks(ticks=y_re_labels)
# axsl[1].plot([],[], "-",color="k", label="Theory")
# axsl[1].errorbar([],[], [], fmt=".", color="k", capsize=3, label="Data")
# axsl[1].legend(loc=10, ncol=2, bbox_to_anchor=(0.5,-0.24), edgecolor="k", facecolor="#ffffe6", framealpha=1, fontsize=10)

axsl[0].set_ylabel("Imaginary part", fontsize=11)
axsl[1].set_ylabel("Real part", fontsize=11)


axsc = [fig.add_subplot(gs[0, 1]),fig.add_subplot(gs[1, 1])]
axsc[0].set_title("$w_{+,2}$", fontsize=11)#"a_2/a_1\\approx$"+str("%.2f" % (a_21),)+")")
...
axsc[0].plot(chi_plt, w2(chi_plt).imag, "-",color=colors[3], alpha=0.8, label="$\Im(w_{2,+})$ theory")
...
#     # ax.set_xticks([-np.pi,-np.pi/2,0,np.pi/2,np.pi,3*np.pi/2])
...
axsr[0].set_title("$w_{+,1}+w_{+,2}$", fontsize=11)#"a_2/a_1\\approx$"+str("%.2f" % (a_21),)+")")
...
axsr[0].plot(chi_plt, w1(chi_plt).imag+w2(chi_plt).imag, "-",color=colors[3], alpha=0.8, label="$\Im(w_{2,+})$ theory")
...
ylim_re_1=-0.2
ylim_re_2=5
y_re_labels=np.arange(ylim_re_1+0.2,ylim_re_2,1)
...
axsr[1].plot(chi_plt, w2(chi_plt).real+w1(chi_plt).real, "-", color=colors[3], alpha=0.8)#, label="Theory")
