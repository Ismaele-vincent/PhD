...
# -*- coding: utf-8 -*-
"""
Created on Wed May 29 16:34:28 2024
...
ax = fig.add_subplot(111)
ax1 = ax.twiny()
# ax1.tick_params(axis="both", labelleft=False, left = False, labelbottom=False, bottom = False)
# ax.tick_params(axis="x", labeltop=False, top = False)
colors=["k","#f10d0c","#00a933","#5983b0"]

ax.errorbar(current*p[0], C, yerr= C_err, fmt="k.", label="Contrast from\ninterferograms")
ax.errorbar([],[], fmt="--", color=colors[2], label="$\propto J_0(\\alpha_1)\,J_0(\\alpha_2)$")
ax.errorbar(curr_plt*p[0], contr(curr_plt,*p), color=colors[0], label="Fit including\nhigher orders")
ax1.errorbar(curr_plt*p[1], j0j0(curr_plt,*p[:3]), fmt="--", color=colors[2], label="$\propto J_0(\\alpha_1)\,J_0(\\alpha_2)$")
ax1.set_xlabel("$\\alpha_2$")
...
plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Images/Alpha1xalpha2_example.pdf", format="pdf",bbox_inches="tight")
plt.show()