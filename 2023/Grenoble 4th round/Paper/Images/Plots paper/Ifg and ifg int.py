...
for i in range(len(ps_int)):
    matrix[i]=tot_data_int[:,5][tot_data_int[:,-1]==ps_int[i]]
...
B0=([10,0,0.01,-10],[np.amax(data_int)+10000,np.amax(data_int)+10000,5, 10])
...
B0=([10,0,0.01,-10],[np.amax(data_ifg)+10000,np.amax(data_ifg)+10000,5, 10])
...
ax.errorbar(ps_int,data_int/p_int[0]*100, yerr=data_int**0.5/p_int[0]*100,fmt="o",color=colors[1], capsize=5, ms=3, label="$\mathcal{N}_+ (\\alpha_{1,2}\\approx\pi/16$)")
