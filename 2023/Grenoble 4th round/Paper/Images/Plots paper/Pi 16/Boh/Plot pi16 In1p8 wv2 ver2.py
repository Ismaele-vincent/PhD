...
for i in range(len(ps_pos)):
    ...
    Im_data_2[i]=c_1_data_2.imag/abs(e_mxi_2.imag)/(cos2[i])/alpha_2
...
xlim1=chi[0]-0.2
xlim2=chi[-7]-0.2
ylabels=np.arange(ylim1,ylim2,0.5)
d=0.02
h=0.05
...
axs[0].set_title("$w_{+,2}$")# $(a_2/a_1\\approx$"+str("%.2f" % (a_21),)+")")
...
axs[0].set_ylim([np.amax(Re_2)-np.amax(Re_err_2)*1.5,np.amax(Re_2)+np.amax(Re_err_2)*1.5])
...
# with open("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Results txt/No In/Wv12_Im In 1p8"+inf_file_name[-10:]+".txt","w") as f:
#     np.savetxt(f,np.transpose([chi,Im_data_1,Im_data_err_1,Im_data_2,Im_data_err_2]), header="chi w_im1 w_im1_err w_im2 w_im2_err")
# with open("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Results txt/No In/Wv12_Re In 1p8"+inf_file_name[-10:]+".txt","w") as f:
