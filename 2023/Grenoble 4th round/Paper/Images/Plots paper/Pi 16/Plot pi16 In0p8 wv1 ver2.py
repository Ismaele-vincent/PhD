...
gs = fig.add_gridspec(3, 1,  height_ratios=(1,1, 6), hspace=0.1)
...
axs[0].set_ylim([np.amax(Re_1)*0.7,np.amax(Re_1)+np.amax(Re_err_1)*1.1])
axs[1].set_ylim([np.amax(Im_data_1)*0.7,np.amax(Im_data_1)+np.amax(Im_data_err_1)*1])
...
axs[2].plot((-d, d), (1+h-d, 1+h+d), **kwargs)
axs[2].plot((1-d, 1+d), (1+h-d, 1+h+d), **kwargs)
axs[2].plot((-d, d), (1.21-d, 1.21+d), **kwargs)
axs[2].plot((1-d, 1+d), (1.21-d, 1.21+d), **kwargs)
axs[2].plot((-d, d), (1.21+h-d, 1.21+h+d), **kwargs)
axs[2].plot((1-d, 1+d), (1.21+h-d, 1.21+h+d), **kwargs)
# ax.legend()
# plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Images/Wv1 In 0p8"+inf_file_name[-10:]+"ver2.pdf", format="pdf",bbox_inches="tight")
