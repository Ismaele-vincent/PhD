...
xlim1=chi[0]-0.2
xlim2=chi[-7]-0.2
ylabels=np.arange(ylim1+1,ylim2+1,1)
d=0.02
h=-0.045
fig = plt.figure(figsize=(5,6))
# fig, axs = plt.subplots(1, 2, figsize=(10, 4))
gs = fig.add_gridspec(3, 1,  height_ratios=(6,1, 1), hspace=0.1)
axs = [fig.add_subplot(gs[1, 0]),fig.add_subplot(gs[2, 0]),fig.add_subplot(gs[0, 0])]
...
axs[2].set_title("$w_{+,2}$")# $(a_2/a_1\\approx$"+str("%.2f" % (a_21),)+")")
...
axs[1].set_ylim([np.amin(Re_2)-np.amax(Re_err_2),np.amin(Re_2)*0.8])
axs[0].set_ylim([np.amin(Im_data_2)-np.amax(Im_data_err_2)*1,np.amin(Im_data_2)*0.8])
axs[2].set_ylim([ylim1,ylim2])
axs[2].set_yticks(ticks=ylabels)
...
kwargs = dict(transform=axs[2].transAxes, color='k', lw=0.8, clip_on=False)
axs[2].plot((-d, d), (-d, +d), **kwargs)
axs[2].plot((1-d, 1+d), (-d, +d), **kwargs)
axs[2].plot((-d, d), (h-d, h+d), **kwargs)
axs[2].plot((1-d, 1+d), (h-d, h+d), **kwargs)
axs[2].plot((-d, d), (-0.21-d, -0.21+d), **kwargs)
axs[2].plot((1-d, 1+d), (-0.21-d, -0.21+d), **kwargs)
axs[2].plot((-d, d), (-0.21+h-d, -0.21+h+d), **kwargs)
axs[2].plot((1-d, 1+d), (-0.21+h-d, -0.21+h+d), **kwargs)
# ax.legend()
plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Images/Wv2 In 0p8"+inf_file_name[-10:]+"ver2.pdf", format="pdf",bbox_inches="tight")
