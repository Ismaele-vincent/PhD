...
xlim1=chi[0]-0.2
xlim2=chi[-7]-0.2
ylabels=np.arange(ylim1+0.5,ylim2+0.5,0.5)
d=0.02
h=-0.06
...
axs[1].set_ylim([np.amin(Re_1)-np.amax(Re_err_1)*1.5,np.amin(Re_1)+np.amax(Re_err_1)*1.5])
# axs[0].set_ylim([ylim1,ylim2])
axs[0].set_yticks(ticks=ylabels)
...
axs[1].plot((-d, d), (-0.01-d, +d-0.01), **kwargs)
axs[1].plot((1-d, 1+d), (-d-0.01, +d-0.01), **kwargs)
axs[1].plot((-d, d), (h-d, h+d), **kwargs)
axs[1].plot((1-d, 1+d), (h-d, h+d), **kwargs)

# ax.legend()
# plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Images/Wv1 In 1p8"+inf_file_name[-10:]+"ver2.pdf", format="pdf",bbox_inches="tight")
