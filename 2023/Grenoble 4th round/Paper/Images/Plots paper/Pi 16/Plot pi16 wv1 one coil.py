...
for i in range(len(ps_pos)):
    ...
    chi_aus=chi[i]
    P0=[np.cos(chi[i]/2)**2, w1(chi[i]).imag*alpha_1, -1]
    # print(P0)
    B0=([0,w1(chi[i]).imag*alpha_1-1000, -2*np.pi],[np.inf, w1(chi[i]).imag*alpha_1+1000, 2*np.pi])
...
ax.set_title("$w_{+,2}$")# $(a_2/a_1\\approx$"+str("%.2f" % (a_21),)+")")
colors=["k","#f10d0c","#00a933","#5983b0"]
ax.errorbar(chi[:-7], Re_1[:-7], Re_err_1[:-7], fmt=".", color=colors[3], capsize=3)
ax.plot(chi_plt[chi_plt<xlim2], w2(chi_plt).real[chi_plt<xlim2], "--", color=colors[3], alpha=0.5)
ax.errorbar(chi, Im_data_1, Im_data_err_1, fmt=".", color=colors[1], capsize=3)
ax.plot(chi_plt, w2(chi_plt).imag, "--",color=colors[1], alpha=0.5)
...
# plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Images/Wv2"+inf_file_name[-10:]+"ver2.pdf", format="pdf",bbox_inches="tight")
