...
for i in range(len(ps_pos)):
    ...
    var=np.sum(func_data_err**2/2)**0.5
    ...
    c_0_data_err=(var**2*2+A_err**2)**0.5
    c_1_data_err_1=var
    c_1_data_err_2=var
...
# fig.suptitle("$a_2/a_1\\approx$"+str("%.2f" % (a_21),))
colors=["k","#f10d0c","#00a933","#5983b0"]
for ax in axs:
    ax.errorbar(chi, Im_data_1, Im_data_err_1, fmt=".", color=colors[0], capsize=3, label="$\Im(w_{1,+})$ data")
    ax.plot(chi_plt, w1(chi_plt).imag, "--", color=colors[0], alpha=0.5, label="$\Im(w_{1,+})$ theory")
...
# axs[0].plot([chi[0],chi[-1]],[ylim1,ylim1], "r", lw=1, ls=(0,(5,3)))
# axs[0].plot([chi[0],chi[-1]],[ylim2,ylim2], "r", lw=1, ls=(0,(5,3)))

kwargs = dict(transform=axs[1].transAxes, color='k', ls="dashed", lw=1, clip_on=False)
axs[1].plot((-0.095, 0), (y1, 1), **kwargs)
axs[1].plot((-0.095, 0), (y2, 0), **kwargs)
kwargs = dict(transform=axs[0].transAxes, color='k', ls="dashed", lw=1, clip_on=False)
axs[1].plot((0, 1), (y1, y1), **kwargs)
axs[1].plot((0, 1), (y2, y2), **kwargs)
# axs[0].legend()
plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Images/Simoultaneous pi16 no In ver2.pdf", format="pdf",bbox_inches="tight")
