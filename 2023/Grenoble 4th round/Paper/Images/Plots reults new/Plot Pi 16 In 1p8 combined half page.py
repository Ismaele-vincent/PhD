...
P0=[(np.amax(cos2_avg)+np.amin(cos2_avg))/2, (np.amax(cos2_avg)-np.amin(cos2_avg))/2, 3, -1]
...
# for ax in axsl:
#     ax.set_xticks([0,np.pi/2,np.pi,3*np.pi/2,2*np.pi,5*np.pi/2])
#     ax.set_xticklabels(["0","$\\dfrac{\pi}{2}$","$\pi$","$\\dfrac{3\pi}{2}$","$2\pi$","$-\\dfrac{\pi}{2}$"])
