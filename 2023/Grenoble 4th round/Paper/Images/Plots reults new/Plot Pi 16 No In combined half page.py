...
for ax in axsl:
    ...
    # ax.set_xticks([-np.pi/2,0,np.pi/2,np.pi,3*np.pi/2,2*np.pi])
    # ax.set_xticklabels(["$-\\dfrac{\pi}{2}$","0","$\\dfrac{\pi}{2}$","$\pi$","$\\dfrac{3\pi}{2}$","$2\pi$"])
...
axsr[0].errorbar(chi, Re_1_avg, Re_1_avg_err, fmt=".", color=colors[0], capsize=3)#, label="Data")
axsr[0].plot(chi_plt, w1(chi_plt).real, "-", color=colors[3], alpha=0.8)#, label="Theory")
...
# axsr[0].set_ylabel("$w_{+,1}$", fontsize=11, rotation=270, va="bottom")
# axsr[1].set_ylabel("$w_{+,2}$", fontsize=11, rotation=270, va="bottom")
# axsr[2].set_ylabel("$w_{+,1}+w_{+,2}$", fontsize=11, rotation=270, va="bottom")
plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Images/Wv No In combined.pdf", format="pdf",bbox_inches="tight")
