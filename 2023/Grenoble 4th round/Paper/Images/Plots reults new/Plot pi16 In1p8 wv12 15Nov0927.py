...
axsl = [fig.add_subplot(gs[0, 0]),fig.add_subplot(gs[1, 0]),fig.add_subplot(gs[2:, 0])]

axsl[1].spines["bottom"].set_visible(False)
axsl[2].spines["left"].set_visible(False)
...
axsl[1].set_ylim([np.amax(Re_1)*0.9,np.amax(Re_1)*1.1])
axsl[2].set_ylim([ylim_re_1,ylim_re_2])
axsl[2].set_yticks(ticks=axsl[2].get_yticks()[1:-1])
axsl[2].set_xlabel("$\\chi$ [rad]")

kwargs = dict(transform=axsl[2].transAxes, color='w', lw=10, clip_on=False)
axsl[2].plot((0, 1), (1-h/2, 1-h/2), **kwargs)

kwargs = dict(transform=axsl[2].transAxes, color='k', lw=0.8, clip_on=False)
# axsl[2].plot((-d, d), (1-d, 1+d), **kwargs)
axsl[2].plot((-d, +d), (1-d, 1+d), **kwargs)
# axsl[2].plot((-d, d), (1+h-d, 1+h+d), **kwargs)
axsl[2].plot((-d, +d), (1-h-d, 1-h+d), **kwargs)
axsl[2].plot((0, 0), (0, 1-h), **kwargs)
...
axsr = [fig.add_subplot(gs[0, 1]), fig.add_subplot(gs[1:, 1])]
...
axsr[1].errorbar(chi[:-7], Re_2[:-7], Re_err_2[:-7], fmt=".", color=colors[3], capsize=3, label="$\Im(w_{1,+})$ data")
axsr[1].plot(chi_plt[chi_plt<xlim2], w2(chi_plt).real[chi_plt<xlim2], "--", color=colors[3], alpha=0.5, label="$\Im(w_{1,+})$ theory")
axsr[1].grid(True, ls="dotted")
axsr[1].tick_params(axis="y", left=False, labelleft=False, right=True, labelright=True)
