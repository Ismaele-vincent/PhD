...
ax.errorbar(ps_pos,ps_data/2000, yerr=ps_data**0.5/2000,fmt="ko",capsize=5, ms=3)
ax.plot(ps_plt,fit_cos(ps_plt, *p_int)/2000, "b")
...
for i in range(len(ps_pos)):
    ...
    Im_data_err_2[i]=(abs(c_1_data_err_2/cos2[i])**2 + (abs((c_1_data_2)/cos2[i]**2)*cos2_err[i])**2+abs((c_1_data_2)/cos2[i]/alpha_2*alpha_2_err)**2)**0.5/abs(alpha_2)
...
gs = fig.add_gridspec(4,2, height_ratios=(7,1.5, 4,1.5), hspace=0.0,wspace=0.0)
...
ax_aus.spines["top"].set_visible(False)
ax_aus.spines["right"].set_visible(False)
ax_aus.spines["bottom"].set_visible(False)
ax_aus.spines["left"].set_visible(False)
axsl = [fig.add_subplot(gs[0, 0]),fig.add_subplot(gs[1:-1, 0]), fig.add_subplot(gs[3, 0]),]

axsl[1].spines["bottom"].set_visible(False)
axsl[1].spines["left"].set_visible(False)
axsl[2].spines["top"].set_visible(False)
...
axsl[0].set_yticks(ticks=axsl[0].get_yticks()[1:-1])
...
axsl[0].yaxis.set_label_coords(-0.3,0.5)
axsl[0].set_ylabel("Imaginary part", fontsize=12)
ax_aus.yaxis.set_label_coords(-0.4,0.5)
ax_aus.set_ylabel("Real part", fontsize=12)

for ax in axsl[1:]:
    ax.errorbar(chi[:-7], Re_1[:-7], Re_err_1[:-7], fmt=".", color=colors[2], capsize=3, label="$\Im(w_{1,+})$ data")
    ax.plot(chi_plt[chi_plt<xlim2], w1(chi_plt).real[chi_plt<xlim2], "--", color=colors[2], alpha=0.5, label="$\Im(w_{1,+})$ theory")
    ax.grid(True, ls="dotted")
    ax.set_xlim([chi[0]-0.2,chi[-1]+0.2])
axsl[1].set_ylim([ylim_re_1,ylim_re_2])
axsl[1].set_yticks(ticks=axsl[1].get_yticks()[1:-1])
axsl[2].set_xlabel("$\\chi$ [rad]")
axsl[2].set_ylim([np.amin(Re_1)*1.4,np.amin(Re_1)*0.6])



kwargs = dict(transform=axsl[1].transAxes, color='w', lw=10, clip_on=False)
axsl[1].plot((-d, 1), (+h/2, +h/2), **kwargs)

kwargs = dict(transform=axsl[1].transAxes, color='k', lw=0.8, clip_on=False)
# axsl[1].plot((-d, d), (1-d, 1+d), **kwargs)
axsl[1].plot((-d, +d), (-d, +d), **kwargs)
# axsl[1].plot((-d, d), (1+h-d, 1+h+d), **kwargs)
axsl[1].plot((-d, +d), (h-d, h+d), **kwargs)
axsl[1].plot((0, 0), (h, 1), **kwargs)

ylim_re_1=0.5
ylim_re_2=2.5
# y_re_labels=np.arange(ylim_re_1,ylim_re_2,02)
ylim_im_1=-1
ylim_im_2=1
y_im_labels=np.arange(ylim_im_1+1,ylim_im_2,1.5)

# ax_aus.set_ylabel("$w^\Re_{+,2}$", fontsize=12, rotation=270, va="bottom", ha="center")
axsr = [fig.add_subplot(gs[0, 1]), fig.add_subplot(gs[1, 1]),fig.add_subplot(gs[2:, 1])]
axsr[1].spines["bottom"].set_visible(False)
axsr[2].spines["top"].set_visible(False)
axsr[2].spines["right"].set_visible(False)

axsr[0].tick_params(axis="x", bottom=False, labelbottom=False)
axsr[1].tick_params(axis="x", bottom=False, labelbottom=False)
axsr[0].set_title("$w_{+,2}$")#"\t$(a_2/a_1\\approx$"+str("%.2f" % (a_21),)+")")
colors=["k","#f10d0c","#00a933","#5983b0"]
axsr[0].tick_params(axis="y", left=False, labelleft=False, right=True, labelright=True)
axsr[0].errorbar(chi, Im_data_2, Im_data_err_2, fmt=".", color=colors[1], capsize=3, label="$\Im(w_{2,+})$ data")
axsr[0].plot(chi_plt, w2(chi_plt).imag, "--",color=colors[1], alpha=0.5, label="$\Im(w_{2,+})$ theory")
axsr[0].set_ylim([ylim_im_1,ylim_im_2])
axsr[0].set_yticks(ticks=axsr[0].get_yticks()[1:-1])
axsr[0].grid(True, ls="dotted")
axsr[0].set_xlim([chi[0]-0.2,chi[-1]+0.2])
axsr[0].yaxis.set_label_position("right")
# axsr[0].set_ylabel("$w^\Im_{+,2}$", fontsize=12, rotation=270, va="bottom", ha="center")
for ax in axsr[1:]:
...
axsr[1].set_ylim([np.amax(Re_2)*0.8,np.amax(Re_2)*1.2])
axsr[2].set_ylim([ylim_re_1,ylim_re_2])
axsr[2].set_yticks(ticks=axsr[2].get_yticks()[1:-1])
axsr[2].set_xlabel("$\\chi$ [rad]")

kwargs = dict(transform=axsr[2].transAxes, color='w', lw=10, clip_on=False)
axsr[2].plot((0.05, 1+d*1.5), (1-h/2, 1-h/2), **kwargs)

kwargs = dict(transform=axsr[2].transAxes, color='k', lw=0.8, clip_on=False)
# axsr[2].plot((-d, d), (1-d, 1+d), **kwargs)
axsr[2].plot((1-d, 1+d), (1-d, 1+d), **kwargs)
# axsr[2].plot((-d, d), (1+h-d, 1+h+d), **kwargs)
axsr[2].plot((1-d, 1+d), (1-h-d, 1-h+d), **kwargs)
axsr[2].plot((1, 1), (0, 1-h), **kwargs)
# plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Images/Wv12 In 1p8"+inf_file_name[-10:]+".pdf", format="pdf",bbox_inches="tight")

with open("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Results txt/In 1p8/Wv12_Im In 1p8"+inf_file_name[-10:]+".txt","w") as f:
    np.savetxt(f,np.transpose([chi,Im_data_1,Im_data_err_1,Im_data_2,Im_data_err_2]), header="chi w_im1 w_im1_err w_im2 w_im2_err")
with open("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Results txt/In 1p8/Wv12_Re In 1p8"+inf_file_name[-10:]+".txt","w") as f:
...
with open("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Results txt/In 1p8/cos2 In 1p8"+inf_file_name[-10:]+".txt","w") as f:
    np.savetxt(f,np.transpose([ps_pos,cos2/20,cos2_err/20]), header="chi cos2 cos2err")
