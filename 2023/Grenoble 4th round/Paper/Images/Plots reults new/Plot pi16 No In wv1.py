...
axs[0].errorbar(chi, Im_data_1, Im_data_err_1, fmt=".", color=colors[0], capsize=3, label="$\Im(w_{1,+})$ data")
axs[0].plot(chi_plt, w1(chi_plt).imag, "--",color=colors[0], alpha=0.5, label="$\Im(w_{1,+})$ theory")
axs[0].set_ylim([ylim_im_1,ylim_im_2])
axs[0].set_yticks(ticks=y_im_labels)
axs[0].grid(True, ls="dotted")
axs[0].set_xlim([chi[0]-0.2,chi[-1]+0.2])
axs[0].yaxis.set_label_position("right")
axs[0].set_ylabel("$w^\Im_{+,1}$", fontsize=12, rotation=270, va="bottom", ha="center")

axs[1].errorbar(chi[:-7], Re_1[:-7], Re_err_1[:-7], fmt=".", color=colors[2], capsize=3, label="$\Im(w_{1,+})$ data")
axs[1].plot(chi_plt[chi_plt<xlim2], w1(chi_plt).real[chi_plt<xlim2], "--", color=colors[2], alpha=0.5, label="$\Im(w_{1,+})$ theory")
axs[1].grid(True, ls="dotted")
axs[1].set_xlim([chi[0]-0.2,chi[-1]+0.2])
axs[1].yaxis.set_label_position("right")
axs[1].set_ylabel("$w^\Re_{+,1}$", fontsize=12, rotation=270, va="bottom", ha="center")
axs[1].set_ylim([ylim_re_1,ylim_re_2])
axs[1].set_yticks(ticks=y_re_labels)
