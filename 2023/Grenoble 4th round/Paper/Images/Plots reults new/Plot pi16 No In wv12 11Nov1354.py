...
for i in range(len(ps_pos)):
    ...
    chi_aus=chi[i]
    P0=[np.cos(chi[i]/2)**2*A*C_id, w1(chi[i]).imag*alpha_1, w2(chi[i]).imag*alpha_2, 2.1, 1.2]
    # print(P0)
    B0=([0,w1(chi[i]).imag*alpha_1-1000, w2(chi[i]).imag*alpha_2-1000, 0, 0],[np.inf, w1(chi[i]).imag*alpha_1+1000, w2(chi[i]).imag*alpha_2+1000, np.pi, np.pi])
    ...
    # e_mxi_1=np.exp(-1j*(np.angle(c_1_data_1)))
    # e_mxi_2=np.exp(-1j*(np.angle(c_1_data_2)))
    xi_1=2.22108922-np.pi/2 
    xi_2=1.16701599-np.pi/2
    # xi_1=2.404544909127107
    e_mxi_1=np.exp(-1j*(xi_1))
    # xi_2=2.6491221178165554
...
fig.suptitle("${a_2/a_1\\approx"+str("%.2f" % (a_21),)+"}$", y=0.95)
gs = fig.add_gridspec(3,2, height_ratios=(7,1, 6), hspace=0.1,wspace=0.1)
...
axsl[0].errorbar(chi, Im_data_2, Im_data_err_2, fmt=".", color=colors[0], capsize=3, label="$\Im(w_{1,+})$ data")
axsl[0].plot(chi_plt, w2(chi_plt).imag, "--",color=colors[0], alpha=0.5, label="$\Im(w_{1,+})$ theory")
