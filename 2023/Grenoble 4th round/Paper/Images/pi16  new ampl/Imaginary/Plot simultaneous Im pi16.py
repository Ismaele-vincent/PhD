...
def fit_Im(t, B, Im_1, Im_2, xi_1, xi_2):
    return B*(1-2*Im_1*alpha_1*np.sin(2*np.pi*1e-3*f_1*t+xi_1)-2*Im_2*alpha_2*np.sin(2*np.pi*1e-3*f_2*t+xi_2))
...
matrix_err_fit=(matrix_err**2/(C_id*A_aus)**2+matrix**2*A_err**2/(A_aus**2*C_id)**2+((matrix/A_aus-0.5)/C_id**2)**2*C_id_err**2)**0.5
