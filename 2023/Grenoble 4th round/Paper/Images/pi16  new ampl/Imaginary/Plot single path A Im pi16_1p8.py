...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 16 17:54:17 2024
...
inf_file_name="TOF_vs_chi_A_In1_22pt_pi16_1200s_13Nov1233"
...
def fit_Im(t, B, Im_1, xi_1):
    return A_aus*((1-C_id)/2+C_id*B*(1-2*Im_1*alpha_1*np.sin(2*np.pi*1e-3*f_1*t+xi_1)))
...
for i in range(len(ps_pos)):
    ...
    cos2[i]=abs(c_0_data)#C_id*(ps_data[i]-A)/(jv(0,alpha_1)*jv(0,alpha_2))#A_aus*C_id*len(time)*np.cos(chi[i]/2)**2#abs(c_0_data)
    cos2_err[i]=abs(c_0_data_err)
...
# plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Paper/Images/Results_A_pi16_In_1p8.pdf", format="pdf",bbox_inches="tight")

# fig = plt.figure(figsize=(8,6), dpi=200)
# ax = fig.add_subplot(111)
# ax.errorbar(ps_pos, cos2, yerr=cos2_err, fmt="k.", capsize=5, label="$c_0$")
# ax.plot(ps_plt, A_aus*C_id*len(time)*np.cos(chi_plt/2)**2*(jv(0,alpha_1)*jv(0,alpha_2))+A_aus*C_id*len(time)*(jv(0,alpha_1)*jv(0,alpha_2)))
