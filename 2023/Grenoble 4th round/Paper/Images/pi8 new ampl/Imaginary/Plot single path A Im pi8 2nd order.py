...
def fit_Im(t, B, Im_1, Re_1, xi_1):
    ...
    # return A_aus*((1-Co)/2+Co*B*(1-2*Im_1*alpha_1*np.sin(2*np.pi*1e-3*f_1*t+xi_1)+alpha_1**2*(Im_1**2+Re_1**2-Re_1)*np.sin(2*np.pi*1e-3*f_1*t+xi_1)**2))
...
for i in range(len(ps_pos)):
    ...
    chi_aus=chi[i]
    P0=[np.cos(chi[i]/2)**2, w1(chi[i]).imag, 0.5, 2]
    # print(P0)
    B0=([0,w1(chi[i]).imag-1000,-100, -2*np.pi],[np.inf, w1(chi[i]).imag+1000, 100, 2*np.pi])
    ...
    Re_data_1_fit[i]=P#0.5*(1+np.sqrt(1+4*P+1j*0).real+np.sqrt(1+4*P+1j*0).imag)
    Re_data_err_1_fit[i]=(err_Im[2]**2+np.sin(chi[i])**2*chi_0_err**2)**0.5
    ...
    Re_data_1[i]=P#0.5*(1+np.sqrt(1+4*P+1j*0).real+np.sqrt(1+4*P+1j*0).imag)
...
# axs[2].plot(chi_plt, abs(w1(chi_plt))**2-w1(chi_plt).real,"r--", alpha=0.5)
# axs[2].errorbar(chi, Re_data_1_fit, Re_data_err_1_fit, fmt="r.", capsize=3)

# Re_chi_pi = Im_data_1*np.tan((chi+np.pi)/2)#-((1+2*a_1*a_2*np.cos(chi))*Im_data_1+(a_1**2-a_2**2)*w2(chi+np.pi).imag)/(2*a_1*a_2*np.sin(chi))
# cot_err=abs(chi_0_err/np.sin(chi/2)**2)
# Re_err=(Im_data_1**2*cot_err**2+np.tan((chi+np.pi)/2)**2*Im_data_err_1**2)**0.5
# axs[1].plot(chi_plt+np.pi, w2(chi_plt+np.pi).real,"b--", alpha=0.5)
# axs[1].errorbar(chi+np.pi, Re_chi_pi,yerr=Re_err, capsize=3,fmt="b.")

# Re_chi_pi_fit = -Im_data_1_fit/np.tan(chi/2)#-((1+2*a_1*a_2*np.cos(chi))*Im_data_1+(a_1**2-a_2**2)*w2(chi+np.pi).imag)/(2*a_1*a_2*np.sin(chi))
# cot_err=abs(chi_0_err/np.sin(chi/2)**2)
# Re_err_fit=(Im_data_1_fit**2*cot_err**2+np.tan((chi+np.pi)/2)**2*Im_data_err_1_fit**2)**0.5
# axs[2].plot(chi_plt+np.pi, w2(chi_plt+np.pi).real,"b--", alpha=0.5)
# axs[2].errorbar(chi+np.pi, Re_chi_pi_fit,yerr=Re_err_fit, capsize=3,fmt="b.")



plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Report/Images/Results_A_pi8_no_In_2nd_order.pdf", format="pdf",bbox_inches="tight")
