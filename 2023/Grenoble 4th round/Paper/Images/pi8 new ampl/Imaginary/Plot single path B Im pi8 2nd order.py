...
A_aus=1
def fit_Im(t, B, Im_2, Re_2, xi_2):
    return A_aus*((1-Co)/2+Co*B*(1-2*Im_2*alpha_2*np.sin(2*np.pi*1e-3*f_2*t+xi_2)+alpha_2**2*(Im_2**2+Re_2**2-Re_2)*np.sin(2*np.pi*1e-3*f_2*t+xi_2)**2))
...
Re_data_2_fit=np.zeros((len(ps_pos)))
Re_data_err_2_fit=np.zeros((len(ps_pos)))
...
for i in range(len(ps_pos)):
    ...
    chi_aus=chi[i]
    P0=[np.cos(chi[i]/2)**2, w2(chi[i]).imag, 0.5, 2]
    # print(P0)
    B0=([0,w1(chi[i]).imag-1000, 0, -2*np.pi],[np.inf, w1(chi[i]).imag+1000, 1, 2*np.pi])
    ...
    Re_data_2_fit[i]=p_Im[2]
    Re_data_err_2_fit[i]=(err_Im[2]**2+np.sin(chi[i])**2*chi_0_err**2)**0.5
    ...
    c_2_data_2=(yf_data[abs(xf-2*f_2)<1/S_F/2]).astype(complex)
    ...
    if (c_1_data_2).real<0:
        # print("here")
        e_mxi_2=np.exp(-1j*(np.angle(c_1_data_2)))
        e_m2xi_2=np.exp(-1j*(np.angle(c_2_data_2)))
    else:
        e_mxi_2=np.exp(-1j*(np.angle(c_1_data_2)+np.pi))
        e_m2xi_2=np.exp(-1j*(np.angle(c_2_data_2+np.pi)))

    cos2[i]=(c_0_data-2*c_2_data_2*e_m2xi_2).real
    cos2_err[i]=(abs(c_0_data_err)**2+4*c_2_data_err_2**2)**0.5    
...
# axs[2].errorbar(chi, Re_data_2_fit, Re_data_err_2_fit, fmt="r.", capsize=3)
