...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 12 12:59:56 2024
...
for i in range(len(ps_pos)):
    ...
    cos2[i]=abs(c_0_data)#Co*(ps_data[i]-A)/(jv(0,alpha_1)*jv(0,alpha_2))#A_aus*Co*len(time)*np.cos(chi[i]/2)**2#abs(c_0_data)
    cos2_err[i]=abs(c_0_data_err)
...
Re_chi_pi_1 = Im_data_1*np.tan((chi+np.pi)/2)#-((1+2*a_1*a_2*np.cos(chi))*Im_data_1+(a_1**2-a_2**2)*w2(chi+np.pi).imag)/(2*a_1*a_2*np.sin(chi))
cot_err=abs(chi_0_err/np.sin(chi/2)**2)
Re_err_1=(Im_data_1**2*cot_err**2+np.tan((chi+np.pi)/2)**2*Im_data_err_1**2)**0.5
axs[1].plot(chi_plt+np.pi, w2(chi_plt+np.pi).real,"b--", alpha=0.5)
axs[1].errorbar(chi+np.pi, Re_chi_pi_1,yerr=Re_err_1, capsize=3,fmt="b.")

Re_chi_pi_fit_1 = Im_data_1_fit*np.tan((chi+np.pi)/2)#-((1+2*a_1*a_2*np.cos(chi))*Im_data_1+(a_1**2-a_2**2)*w2(chi+np.pi).imag)/(2*a_1*a_2*np.sin(chi))
cot_err=abs(chi_0_err/np.sin(chi/2)**2)
Re_err_fit_1=(Im_data_1_fit**2*cot_err**2+np.tan((chi+np.pi)/2)**2*Im_data_err_1_fit**2)**0.5
axs[2].plot(chi_plt+np.pi, w2(chi_plt+np.pi).real,"b--", alpha=0.5)
axs[2].errorbar(chi+np.pi, Re_chi_pi_fit_1,yerr=Re_err_fit_1, capsize=3,fmt="b.")


Re_chi_pi_2 = -Im_data_2*np.tan((chi+np.pi)/2)#-((1+2*a_1*a_2*np.cos(chi))*Im_data_1+(a_1**2-a_2**2)*w2(chi+np.pi).imag)/(2*a_1*a_2*np.sin(chi))
cot_err=abs(chi_0_err/np.sin(chi/2)**2)
Re_err_2=(Im_data_2**2*cot_err**2+np.tan((chi+np.pi)/2)**2*Im_data_err_2**2)**0.5
axs[1].plot(chi_plt+np.pi, w2(chi_plt+np.pi).real,"r--", alpha=0.5)
axs[1].errorbar(chi+np.pi, Re_chi_pi_2,yerr=Re_err_2, capsize=3,fmt="r.")

Re_chi_pi_fit_2 = -Im_data_2_fit*np.tan((chi+np.pi)/2)#-((1+2*a_1*a_2*np.cos(chi))*Im_data_1+(a_1**2-a_2**2)*w2(chi+np.pi).imag)/(2*a_1*a_2*np.sin(chi))
cot_err=abs(chi_0_err/np.sin(chi/2)**2)
Re_err_fit_2=(Im_data_2_fit**2*cot_err**2+np.tan((chi+np.pi)/2)**2*Im_data_err_2_fit**2)**0.5
axs[2].plot(chi_plt+np.pi, w2(chi_plt+np.pi).real,"r--", alpha=0.5)
axs[2].errorbar(chi+np.pi, Re_chi_pi_fit_2,yerr=Re_err_fit_2, capsize=3,fmt="r.")

plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Report/Images/Results_pi8_no_In_Re.pdf", format="pdf",bbox_inches="tight")
