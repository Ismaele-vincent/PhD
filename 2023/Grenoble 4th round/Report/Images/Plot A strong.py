...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 16 11:53:40 2024
...
for i in range(len(ps_pos)):
    ...
    chi_aus=chi[i]
    P0=[np.cos(chi[i]/2)**2, w1(chi[i]).imag, 0.5, 2.6]
    # print(P0)
    ...
    cos2[i]=(abs(c_0_data)-A_aus*len(time)/2)/(jv(0,alpha_1))+A_aus*len(time)/2-A#(abs(c_0_data))#-A_aus*len(time)*Co/2)/(jv(0,alpha_1))+A_aus*len(time)*Co/2
    cos2_err[i]=abs(c_0_data_err)
    
    Im_data_1[i]=(c_1_data_1*e_m1xi).real/(cos2[i])/alpha_1
    Im_data_err_1[i]=(abs(c_1_data_err_1/cos2[i])**2 + (abs((c_1_data_1*e_m1xi)/cos2[i]**2)*cos2_err[i])**2+abs((c_1_data_1*e_m1xi)/cos2[i]/alpha_1*alpha_1_err)**2)**0.5/abs(alpha_1)
...
# plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Report/Images/Results_pi8_no_In.pdf", format="pdf",bbox_inches="tight")
