...
def fit_Im(t, B, Im_1, Re_1, xi_1):
    # return A_aus*((1-Co)/2+Co*B*(1+2*Re_1*alpha_1**2*np.sin(2*np.pi*1e-3*f_1*t+xi_1)**2-2*Im_1*alpha_1*np.sin(2*np.pi*1e-3*f_1*t+xi_1)))
...
P0=[(np.amax(ps_data)+np.amin(ps_data))/2, (np.amax(ps_data)-np.amin(ps_data))/2, 3, -0.3]
...
fig.suptitle("$\mathbf{\\alpha=\pi/2}$",bbox=dict(facecolor='none', edgecolor='k'))
...
for i in range(len(ps_pos)):
    ...
    Re_data_err_1_fit[i]=err_P#4/(Re_data_1_fit[i]-0.5)**0.5*err_P
...
axs[2].plot(chi_plt, abs(w1(chi_plt).real)**2-w1(chi_plt).real,"r--", alpha=0.5)
...
plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Report/Images/Results_A_pi2_no_In_strong.pdf", format="pdf",bbox_inches="tight")
