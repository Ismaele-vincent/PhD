...
gs = GridSpec(2,2, figure=fig, wspace=0, hspace=0, top=1, bottom=0)
axs=[fig.add_subplot(gs[:,:]), fig.add_subplot(gs[0,0]),fig.add_subplot(gs[0,1]), fig.add_subplot(gs[1,0]),fig.add_subplot(gs[1,1])]
...
for j in [0,1]:
    for i in range(len(ps_pos)):
        ...
        chi_aus=chi[i]
        P0=[np.cos(chi[i]/2)**2, w1(chi[i]).imag, w2(chi[i]).imag, 2, -1]
        # print(P0)
        ...
        Im_data_1[i]=(c_1_data_1*e_m1xi).real/(cos2[i])/alpha_1
    ...
    axs[3-2*j].plot(chi_plt, w1(chi_plt).imag,"k--", alpha=0.5)
    axs[3-2*j].errorbar(chi, Im_data_1, Im_data_err_1, fmt="k.",capsize=3)
    axs[4-2*j].plot(chi_plt, w1(chi_plt).imag,"k--", alpha=0.5)
    axs[4-2*j].errorbar(chi, Im_data_1_fit, Im_data_err_1_fit, fmt="k.", capsize=3)
    ...
    axs[3-2*j].plot(chi_plt, w2(chi_plt).imag,"g--", alpha=0.5)
    axs[3-2*j].errorbar(chi, Im_data_2, Im_data_err_2, fmt="g.", capsize=3)
    axs[4-2*j].plot(chi_plt, w2(chi_plt).imag,"g--", alpha=0.5)
    axs[4-2*j].errorbar(chi, Im_data_2_fit, Im_data_err_2_fit, fmt="g.", capsize=3)
...
plt.savefig("/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/Report/Images/Correction_example_pi16.pdf", format="pdf",bbox_inches="tight")
