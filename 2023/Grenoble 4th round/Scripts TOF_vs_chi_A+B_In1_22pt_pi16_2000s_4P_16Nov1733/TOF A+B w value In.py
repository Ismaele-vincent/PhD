# -*- coding: utf-8 -*-
"""
Created on Wed Nov 15 13:47:46 2023
...
def fit_cos(x, A, B, C, D):
    return A/2+B/2*jv(0,alpha_1)**2*np.cos(C*x-D)

def alpha(T,f,B):
    w=f*2*np.pi
    return mu_N*B/(hbar*w)*2*np.sin(w*T*1e-3/2)

A_aus=250.47512286606747
def fit_Im(t, B, Im_1, Im_2, xi_1, xi_2):
    return A_aus*((1-Co)/2+Co*B*(1-2*Im_1*alpha_1*np.sin(2*np.pi*1e-3*f_1*t+xi_1)-2*Im_2*alpha_1*np.sin(2*np.pi*1e-3*f_2*t+xi_2)))
...
P0=[(np.amax(ps_data)+np.amin(ps_data)), (np.amax(ps_data)-np.amin(ps_data)), 3, -3]
B0=([1000,0,0.01,-10],[np.amax(ps_data)+10000,np.amax(ps_data)+10000,5, 10])
...
A_corr=p[0]*(1-Co)/2
A_aus=p[0]/len(time)
print("C=",Co, "w_ps=",w_ps, "A_aus=", A_aus, "A_corr=", A_corr)
...
Im_data_fit1=np.zeros((len(ps_pos)))
Im_data_fit_err1=np.zeros((len(ps_pos)))

Im_data_fit2=np.zeros((len(ps_pos)))
Im_data_fit_err2=np.zeros((len(ps_pos)))
...
for i in range(len(ps_pos)):
    ...
    chi_aus=chi[i]
    P0=[np.cos(chi[i]/2)**2, w2(chi[i]).imag, -w2(chi[i]).imag, 0.9, 0.9]
    # print(P0)
    B0=([0,-2, -2, -2*np.pi, -2*np.pi],[np.inf, 2, 2, 2*np.pi, 2*np.pi])
    p_Im,cov_Im = fit(fit_Im, time, func_data, p0=P0, bounds=B0)
    err_Im=np.diag(cov_Im)**0.5
    # print(p_Im[0])
    # print(p_Im,err_Im)
    Im_data_fit1[i]=p_Im[1]
    Im_data_fit_err1[i]=(err_Im[1]**2+np.sin(chi[i])**2*chi_0_err**2)**0.5
    
    Im_data_fit2[i]=p_Im[2]
    Im_data_fit_err2[i]=(err_Im[2]**2+np.sin(chi[i])**2*chi_0_err**2)**0.5
    ...
    if i==0:
        ...
        x_2=f_2
        print(x_1)
    c_0_data=abs(yf_data[abs(xf)<1/S_F/2]).astype(complex)-A_corr#-3963.5934009557927 #-3421.187068197044
...
ax.plot(chi_plt+np.pi, w1(chi_plt+np.pi).real,"r--")
...
ax.plot(chi_plt+np.pi, w2(chi_plt+np.pi).real,"k--", alpha=0.5)
...
ax.plot(chi_plt, w1(chi_plt).imag,"b--", alpha=0.5)
ax.errorbar(chi, Im_data_1, Im_data_err_1, fmt="b.", capsize=3)
ax.errorbar(chi, Im_data_fit1, Im_data_fit_err1, fmt="y.", capsize=3)

ax.plot(chi_plt, w2(chi_plt).imag,"g--", alpha=0.5)
ax.errorbar(chi, Im_data_2, Im_data_err_2, fmt="g.", capsize=3)
ax.errorbar(chi, Im_data_fit2, Im_data_fit_err2, fmt="m.", capsize=3)
...
# ax.errorbar(ps_pos, ps_data, yerr=ps_data**0.5, fmt="m.", capsize=5, label="$c_0$")
