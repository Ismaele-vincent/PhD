...
bad_apples=["TOF_vs_chi_A+B_22pt_pi16_1200s_09Nov1714","TOF_vs_chi_A+B_22pt_pi16_1200s_09Nov1749","TOF_vs_chi_A+B_In1_22pt_pi16_1200s_4P_14Nov1045"]

for root, dirs, files in os.walk(sc_fold_path, topdown=False):
    for name1 in files:
        if ("TOF_vs_chi_A+B_" in name1) and (".inf" in name1):
                    ...
                    for name in files:
                        if (name in dat_files[:,0]):
                            ...
                            with open(cleandata+"/TOF_ps_"+str("%02d" % (ps_num[0][ps_num[1]==ps][0],)[0])+".txt", 'w') as f:
                                    np.savetxt(f, cleantxt, header= "BinNR microsec tot-time (s)  O-Beam H-beam AUX Ampl1(V) f1(Hz) phase1(deg) Ampl1(V) f1(Hz) phase1(deg) chi(pos)", fmt='%i %.2f %.2f %i %i %i %.3f %.3f %.3f %.3f %.3f %.3f %.3f')