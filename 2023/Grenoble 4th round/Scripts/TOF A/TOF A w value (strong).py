# -*- coding: utf-8 -*-
"""
Created on Wed Nov  8 09:58:28 2023
...
a_1=1/2**0.5
a_2=1/2**0.5
# inf_file_name="TOF_vs_chi_A_19pt_pi16_1200s_05Nov1903"
...
def fit_cos(x, A, B, C, D):
    return A+B*jv(0,alpha_1)*np.cos(C*x-D)
...
for i in range(len(ps_pos)):
    ...
    if i==0:
        ...
        print(x_1)
    c_0_data=abs(yf_data[abs(xf)<1/S_F/2]).astype(complex)-5103.274676652812
    ...
    cos2[i]=(c_0_data+2*(c_2_data/e_m2xi)).real
    cos2_err[i]=(c_0_data_err**2+4*c_2_data_err**2)**0.5
    ...
    # Re_data[i]=0.5+0.5*abs(1-4*(Im_data[i]**2 +2*(c_0_data.real/cos2[i]-1)/alpha_1**2))**0.5
    # Re_data[i]=1/2*(1+(1-4*(Im_data[i]**2 - 4*(c_2_data/e_m2xi).real/(cos2[i]*alpha_1**2)))**0.5)
