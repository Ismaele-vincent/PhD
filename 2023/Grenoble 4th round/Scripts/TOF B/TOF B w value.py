...
# inf_file_name="TOF_vs_chi_B_In1_22pt_pi16_1200s_14Nov1116"
...
def fit_cos(x, A, B, C, D):
    return A+jv(0,alpha_2)*B*np.cos(C*x-D)
...
for i in range(len(ps_pos)):
    ...
    if i==0:
        x_1=f_2#xf[xf>0][abs(yf_data[xf>0])==np.amax(abs(yf_data[xf>0]))]
        print(x_1)
    c_1_data=(yf_data[abs(xf-x_1)<1/S_F/2]).astype(complex)
    c_0_data=abs(yf_data[abs(xf)<1/S_F/2]).astype(complex)-3781.9412544666925
    var=np.sum(func_data)**0.5/2
    c_1_data_err=var
    c_0_data_err=(var**2+283.1369572490254**2)**0.5
