...
bad_apples=["B_test_long_01Nov1802","B_test_long_2kHz_02Nov1519"]

for root, dirs, files in os.walk(sc_fold_path, topdown=False):
    for name1 in files:
        if ("B_test_long" in name1) and (".inf" in name1):
            if (name1[:-4] in bad_apples):
                ...
                sorted_fold_path="/home/aaa/Desktop/Fisica/PhD/2023/Grenoble 4th round/exp_CRG-3061/Sorted data/Alpha Fourier/"+inf_file_name
                    ...
                    for name in files:
                        if (name in dat_files[:,0]):
                            ...
                            with open(cleandata+"/TOF_current_"+str("%02d" % (current[0][dat_files[:,0]==name][0],))+".txt", 'w') as f:
                                    np.savetxt(f, cleantxt, header= "BinNR microsec tot-time (s)  O-Beam H-Beam AUX Amplitude(func. gen.)", fmt='%i %.2f %.2f %i %i %i %.3f')
