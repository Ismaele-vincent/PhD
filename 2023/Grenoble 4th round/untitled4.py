...
inf_file_name_1="ifgPS1_35pt_In10_15Apr1050" #good
...
print(inf_file_name_1)
sorted_fold_path_1="/home/aaa/Desktop/Fisica/PhD/2024/Grenoble 1st round/exp_CRG-3125/Sorted data/Ifg/"+inf_file_name_1
cleandata_1=sorted_fold_path_1+"/Cleantxt"
...
C_avg=0
C_err=0
for root, dirs, files in os.walk(cleandata_1, topdown=False):
    files=np.sort(files)
    data_ifg_matrix_1=np.zeros((4,points))
    for name in files:
        # print(name)
        tot_data=np.loadtxt(os.path.join(root, name))
        data_ifg=tot_data[:,2]
        data_ifg_matrix_1[i]=data_ifg
...
data_ifg_matrix_1_err=(data_ifg_matrix_1+((1-C_id)/2)**2*A_err**2+(A_avg/2)**2*C_err**2)**0.5
# data_ifg_matrix_1-=A_avg*(1-C_id)
data_ifg_matrix_1=(data_ifg_matrix_1-A_avg*(1-C_id)*4)/(A_avg*C_id*4)+1/2
# data_ifg_matrix_1*=A_avg
P1_corr=C_id*P1
P1_corr_err=(P1+C_err**2)**0.5
fig = plt.figure(figsize=(8,6))
ax = fig.add_subplot(111)
fig.suptitle(name[:-4])
ax.errorbar(ps_pos,data_ifg_matrix_1[0],yerr=data_ifg_err*0,fmt="ko",capsize=5, ms=3)
...
# a_21=a_2_2/a_1_2
lim=0
inf_file_name_2="ifgPS1_35pt_In15_13Apr2126" #very good
...
sorted_fold_path_2="/home/aaa/Desktop/Fisica/PhD/2024/Grenoble 1st round/exp_CRG-3125/Sorted data/Ifg/"+inf_file_name_2
cleandata_2=sorted_fold_path_2+"/Cleantxt"
chi_0=[0, np.pi/2, np.pi, 3*np.pi/2]
i=0
A_avg=0
Dchi=np.zeros(4)
C_avg=0
C_err=0
for root, dirs, files in os.walk(cleandata_2, topdown=False):
    files=np.sort(files)
    data_ifg_matrix_2=np.zeros((4,points))
    for name in files:
        # print(name)
        tot_data=np.loadtxt(os.path.join(root, name))
        data_ifg=tot_data[:,2]
        data_ifg_matrix_2[i]=data_ifg
        data_ifg_err=data_ifg**0.5
        ps_pos=tot_data[:,0]
        P0=[(np.amax(data_ifg)+np.amin(data_ifg))/2, (np.amax(data_ifg)-np.amin(data_ifg))/2, 3, -0.6+chi_0[i]]
        B0=([np.amin(data_ifg),0,0.01,-3.5],[np.amax(data_ifg)*2,np.amax(data_ifg)*2,5, 2*np.pi])
...
C_id=C_avg/(2*a_1_2*a_2_2)
C_id_err=(C_err**2+C_avg**2/(a_1**2)*a_1_err**2+C_avg**2/(a_2**2)*a_2_err**2)**0.5/(2*a_1*a_2)
print("C_avg=",C_avg, "+-",C_err, "C_ideal=", C_id, "+-", C_id_err)
alpha=(1-0.66)**0.5# a_1= 0.821
# a_1_err=0.004
# a_2= 0.570
# a_2_err=0.006
data_ifg_matrix_2_err=(data_ifg_matrix_2+((1-C_id)/2)**2*A_err**2+(A_avg/2)**2*C_err**2)**0.5
# data_ifg_matrix_2-=A_avg*(1-C_id)
data_ifg_matrix_2=(data_ifg_matrix_2-A_avg*(1-C_id)*4*(a_1_1**2*(1-alpha)**2+a_2_1**2))/(A_avg*C_id*4)+1/2
# data_ifg_matrix_2*=A_avg
P1_corr=C_id*P1
P1_corr_err=(P1+C_err**2)**0.5
fig = plt.figure(figsize=(8,6))
ax = fig.add_subplot(111)
fig.suptitle(name[:-4])
ax.errorbar(ps_pos,data_ifg_matrix_2[0],yerr=data_ifg_err*0,fmt="ko",capsize=5, ms=3)
print(data_ifg_matrix_1[0]/data_ifg_matrix_2[0])
fig = plt.figure(figsize=(8,6))
ax = fig.add_subplot(111)
fig.suptitle(name[:-4])
ax.errorbar(ps_pos,(data_ifg_matrix_2[0]-data_ifg_matrix_1[0])/data_ifg_matrix_1[0]/2/alpha,yerr=data_ifg_err*0,fmt="ko",capsize=5, ms=3)
ax.plot(ps_pos,w1(chi, a_21).real)
