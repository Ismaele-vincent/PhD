...
for root1, dirs1, files1 in os.walk(sc_fold_path, topdown=False):
    for name1 in files1:
        if ("ifg_vs_alpha1xalpha2" in name1) and (".inf" in name1):
                ...
                for root, dirs, files in os.walk(sc_fold_path, topdown=False):
                    ...
                    for name in files:
                        if (name in dat_files[:,0]):
                            ...
                            Vpp_aux1=dat_files[:,-3][dat_files[:,0]==name].astype(float)
                            Vpp_aux2=dat_files[:,-5][dat_files[:,0]==name].astype(float)#phashifter position
                            # print(curr)
                            cleantxt =np.hstack((txt,np.ones((len(txt[:,0]),1))*Vpp_aux1,np.ones((len(txt[:,0]),1))*Vpp_aux2))
                            # print(Vpp_aux)
                            with open(cleandata+"/ifg_vs_Vpp1_"+str("%02d" % (Vpp[0][dat_files[:,0]==name],))+".txt", 'w') as f:
                                    np.savetxt(f, cleantxt, header= "BinNR ps_pos tot-time (s)  O-Beam H-Beam AUX am2 am1", fmt='%f %.1f %i %i %i %i %i %i %f %f %f %f')
