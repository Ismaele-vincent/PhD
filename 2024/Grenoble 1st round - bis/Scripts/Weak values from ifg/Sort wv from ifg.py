...
for root, dirs, files in os.walk(sc_fold_path, topdown=False):
    files=np.sort(files)
    for name in files:
        if ("ifg_Indium05" in name):#(("ifg_" in name) or ("movePS" in name)) and (".inf" in name) and ("TOF_vs_chi" not in name) and ("alpha" not in name) :
            if (name[:-4] in bad_apples):
                ...
                dat_files = np.genfromtxt(inf_file_path, dtype=str, usecols=0)[1::2]
                # print(dat_files)
                ...
                for root1, dirs1, files1 in os.walk(sc_fold_path, topdown=False):
                    ...
                    for name in files1:
                        if (name in dat_files):
                            ...
                            cleantxt=np.loadtxt(os.path.join(root, name), encoding='windows-1252', comments="*", skiprows=26,delimiter="\t")[:,1:]
