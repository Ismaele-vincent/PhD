...
sa=data_ifg_matrix[2]*data_ifg_matrix[0]-(data_ifg_matrix[3]-data_ifg_matrix[1])**2/4
s=np.sign(sa)*abs(sa)**0.5
ReKD_2=(data_ifg_matrix[0]-s)/(2*(data_ifg_matrix[0]+data_ifg_matrix[2]))
Aux=data_ifg_matrix[0]/(data_ifg_matrix[0]+data_ifg_matrix[2])
Delta_aux=((data_ifg_matrix[2]*data_ifg_matrix_err[0])**2+data_ifg_matrix_err[2]**2)**0.5/(data_ifg_matrix[0]+data_ifg_matrix[2])**2

ReKD_2_err=((Re_1_1*Delta_aux)**2 + (Aux*Re_1_1_err)**2)**0.5
...
axs[0].errorbar(S_x,ReKD_2[((chi > np.pi/2)) & (chi < np.pi)], ReKD_2_err[((chi > np.pi/2)) & (chi < np.pi)], fmt="k.", capsize=3, label="Data")
axs[0].errorbar(2*a_1*a_2*np.cos(chi_plt), a_2**2/2+a_1*a_2*np.cos(chi_plt)/2, color=colors[3], alpha=0.8, label="Theory")
axs[0].plot([-1,0],[1,1], color=colors[1], label = "Negativity\nbound")
axs[0].plot([-1,0],[0,0], color=colors[1])
axs[0].fill_between([-1,0], 1, 5, color=colors[1], alpha= 0.05)
axs[0].fill_between([-1,0], -5, 0, color=colors[1], alpha= 0.05)
...
axs[1].plot([-1,0],[1,1], color=colors[1], label = "Eigenvalue\nbound")
axs[1].plot([-1,0],[0,0], color=colors[1])
axs[1].fill_between([-1,0], 1, 5, color=colors[1], alpha= 0.05)
axs[1].fill_between([-1,0], -5, 0, color=colors[1], alpha= 0.05)
# axs[1].fill_between(2*a_1*a_2*np.cos(chi_plt)+2*a_1*a_2, 1, w1(chi_plt+np.pi/2, a_21).real, w1(chi_plt+np.pi/2, a_21).real>1, color=colors[1], alpha= 0.05)
# axs[1].fill_between(2*a_1*a_2*np.cos(chi_plt), 1, w1(chi_plt+np.pi, a_21).real, w1(chi_plt+np.pi, a_21).real>1, color=colors[1], alpha= 0.05)
# axs[1].xaxis.set_inverted(True)

axs[2].errorbar(S_x, S_z+abs(S_x)*0,  xerr=S_x_err, yerr=S_z_err+abs(S_x)*0,  fmt="k.", capsize=2, label="Data")
# axs[2].plot([0,1],[1,0], "r")
axs[2].plot([-1,0],[0,1], color=colors[1], label = "Noncontextual\nbound")
axs[2].plot([-1,0],[0,-1], color=colors[1])
axs[2].plot(x_c,(1-x_c**2)**0.5, color=colors[2], label = "Quantum bound")
axs[2].plot(x_c,-(1-x_c**2)**0.5, color=colors[2])
axs[2].set_xlim([-1,0])
axs[2].set_ylim([0,1])
axs[2].set_ylabel("$\langle\\sigma_z\\rangle$",fontsize=13)
axs[2].set_xlabel("$\langle\\sigma_x\\rangle$",fontsize=13)
axs[2].set_aspect('equal', 'box')
axs[2].fill_between(x_c, -1-x_c, -(1-x_c**2)**0.5, x_c<0, color=colors[1], alpha= 0.05)
axs[2].fill_between(x_c, 1+x_c, (1-x_c**2)**0.5, x_c<0,  color=colors[1], alpha= 0.05)
