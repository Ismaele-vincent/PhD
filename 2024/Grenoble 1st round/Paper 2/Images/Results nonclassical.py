...
data_ifg_matrix_corr=data_ifg_matrix-A_avg*(1-C_id)
P1_corr=C_id*P1
P1_corr_err=(C_id**2*P1+P1**2*C_id_err**2)**0.5
P2_corr=C_id*P2
P2_corr_err=(C_id**2*P2+P2**2*C_id_err**2)**0.5

Re_Q_1p2= (data_ifg_matrix_corr[0]-data_ifg_matrix_corr[2])/(4*(data_ifg_matrix_corr[0]+data_ifg_matrix_corr[2]))
Im_Q_p1= (data_ifg_matrix_corr[3]-data_ifg_matrix_corr[1])/(4*(data_ifg_matrix_corr[3]+data_ifg_matrix_corr[1]))
Re_Q_p1= P1_corr/(data_ifg_matrix_corr[0]+data_ifg_matrix_corr[2]) + Re_Q_1p2
w_p1_re=(data_ifg_matrix_corr[0]-data_ifg_matrix_corr[2])/(4*(data_ifg_matrix_corr[0]))+P1_corr/(data_ifg_matrix_corr[0])
Re_Q_p2= P2_corr/(data_ifg_matrix_corr[0]+data_ifg_matrix_corr[2]) + Re_Q_1p2
w_p2_re=(data_ifg_matrix_corr[0]-data_ifg_matrix_corr[2])/(4*(data_ifg_matrix_corr[0]))+P2_corr/(data_ifg_matrix_corr[0])

w_p1_re_err=(P1_corr_err**2+(data_ifg_matrix_err[2]/4)**2+(w_p1_re-1/4)**2*data_ifg_matrix_err[0]**2)**0.5/abs(data_ifg_matrix[0])

fig = plt.figure(figsize=(5,6), dpi=150)
ax = fig.add_subplot(111)
# ax.errorbar(chi,Re_Q_1p2, fmt=".-")
# ax.errorbar(chi,Im_Q_1p2, fmt=".-")
ax.errorbar(chi_plt, a_1*a_2*np.cos(chi_plt)/2+a_1**2/2, color=colors[0])
ax.errorbar(chi,Re_Q_p1, fmt="o", color=colors[2])
ax.errorbar(chi_plt, a_1*a_2*np.sin(chi_plt)/2, color=colors[0])
ax.errorbar(chi,Im_Q_p1, fmt="v", color=colors[2])
ax.errorbar(chi_plt, a_1*a_2*np.cos(chi_plt)/2+a_2**2/2, color=colors[0])
ax.errorbar(chi,Re_Q_p2, fmt="o", color=colors[3])
ax.errorbar(chi,-Im_Q_p1, fmt="v", color=colors[3])
ax.errorbar(chi_plt, -a_1*a_2*np.sin(chi_plt)/2, color=colors[0])
xlim=ax.get_xlim()
# ylim0=-0.11
# ylim=[ylim0, ax.get_ylim()[1]]
ax.plot(xlim, [0,0], "-", color=colors[1])
ax.set_xlim(xlim)
# ax.set_ylim(ylim)
ax.fill_between(chi_plt, 0, a_1*a_2*np.cos(chi_plt)/2+a_2**2/2, (a_1*a_2*np.cos(chi_plt)/2+a_2**2/2)<0, color= colors[1],alpha= 0.2)
# ax.errorbar(chi,w_p1_re, fmt="o")
ax.set_xticks([-np.pi,0,np.pi])
ax.set_xticklabels(["$\mathdefault{-\pi}$", "$\mathdefault{0}$","$\mathdefault{\pi}$"])
ax.grid(True, ls="dotted")# C_id=C_avg/(2*a_1*a_2)

fig = plt.figure(figsize=(5,6), dpi=150)
ax = fig.add_subplot(111)
# ax.errorbar(chi,Re_Q_1p2, fmt=".-")
# ax.errorbar(chi,Im_Q_1p2, fmt=".-")
# ax.errorbar(chi,Re_Q_1p, fmt=".-", color=colors[2])
ax.errorbar(chi,w_p1_re, fmt="o", color=colors[2])
ax.errorbar(chi,w_p2_re, fmt="o", color=colors[3])
xlim=ax.get_xlim()
ylim0=-1.5
ylim1=2.8
ylim=[ylim0, ylim1]
ax.plot(xlim,[0,0], "-", color=colors[1])
ax.plot(xlim,[1,1], "-", color=colors[1])
ax.set_xlim(xlim)
# ax.set_ylim(ylim)
ax.fill_between(chi_plt, 1, w1(chi_plt, a_21).real, w1(chi_plt, a_21).real>1, color= colors[1], alpha= 0.1)
ax.fill_between(chi_plt, 0, w2(chi_plt, a_21).real, w2(chi_plt, a_21).real<0, color= colors[1], alpha= 0.1)
ax.errorbar(chi_plt, w1(chi_plt, a_21).real, color=colors[0], alpha=0.8)
ax.errorbar(chi_plt, w2(chi_plt, a_21).real, color=colors[0], alpha=0.8)
ax.set_xticks([-np.pi,0,np.pi])
ax.set_xticklabels(["$\mathdefault{-\pi}$", "$\mathdefault{0}$","$\mathdefault{\pi}$"])
