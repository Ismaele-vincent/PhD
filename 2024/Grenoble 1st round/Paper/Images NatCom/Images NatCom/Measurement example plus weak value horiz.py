...
fig_1 = plt.figure(figsize=(8,4), dpi=250)
gs_l = fig.add_gridspec(1,3, hspace=0.0, wspace=0.4)
gs_r = fig.add_gridspec(1,3, hspace=0.0, wspace=0.4)
gs_1_l = fig_1.add_gridspec(1,3, hspace=0.0, wspace=0.4)
gs_1_r = fig_1.add_gridspec(1,3, hspace=0.0, wspace=0.4)
axs = [fig_1.add_subplot(gs_1_l[0, 0]),fig.add_subplot(gs_l[0, 0]),fig_1.add_subplot(gs_1_l[0, 1]),fig.add_subplot(gs_l[0, 1]),fig_1.add_subplot(gs_1_r[0, 2]),fig.add_subplot(gs_r[0, 2])]
...
for root, dirs, files in os.walk(cleandata, topdown=False):
    ...
    for name in (files[::-1]):
        ...
        if i==0:
            ...
            chi_plt=np.linspace(-np.pi,np.pi,200)
        ...
        axs[(i+2)%4].errorbar(chi_plt,fit_cos(chi_plt/p[2], *p)/time, fmt="--", color=colors[3], label="Theory")
        # axs[(i+2)%4].text(1.05,0.5,labels[i], rotation=-90, ha="center", va="center", transform=axs[i].transAxes)
        axs[(i+2)%4].set_ylabel("[count per $\mathrm{s}^{-1}$]")
        axs[(i+2)%4].set_title(labels[i])
...
for ax in axs:
    ...
    ax.set_xticklabels(["${-\pi}$","${-\\dfrac{\pi}{2}}$", "${0}$","${\\dfrac{\pi}{2}}$","${\pi}$"])
...
axs[4].set_ylabel("Real part")
axs[5].set_ylabel("Imaginary part")
axs[4].set_title("Weak value")
axs[5].set_title("Weak value")
