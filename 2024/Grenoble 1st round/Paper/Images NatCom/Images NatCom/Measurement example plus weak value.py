...
gs = fig.add_gridspec(3,2, hspace=0.0, wspace=0.4)
axs = [fig.add_subplot(gs[0, 1]),fig.add_subplot(gs[0, 0]),fig.add_subplot(gs[1, 1]),fig.add_subplot(gs[1, 0]),fig.add_subplot(gs[2, 0]),fig.add_subplot(gs[2, 1])]
...
for root, dirs, files in os.walk(cleandata, topdown=False):
    ...
    for name in (files[::-1]):
        ...
        axs[(i+2)%4].set_ylabel("Interferogram "+labels[i]+"\n[count per $\mathrm{s}^{-1}$]")
...
axs[1].errorbar(chi[8],(data_ifg_matrix[0])[8]/time, fmt="o",color=colors[1], ms=15, mfc="none")
# axs[1].vlines(chi[8],0,(data_ifg_matrix[0])[8]/time-12, color=colors[1],  lw=1) #ls=(2, (8, 3)),
axs[3].errorbar(chi[8],(data_ifg_matrix[2])[8]/time, fmt="o",color=colors[1], ms=15, mfc="none")
...
axs[0].errorbar(chi[8],(data_ifg_matrix[1])[8]/time, fmt="o",color=colors[2], ms=15, mfc="none")
# axs[0].vlines(chi[8],0,(data_ifg_matrix[1])[8]/time-12, color=colors[1],  lw=1) #ls=(2, (8, 3)),
axs[2].errorbar(chi[8],(data_ifg_matrix[3])[8]/time, fmt="o",color=colors[2], ms=15, mfc="none")
...
axs[4].errorbar(chi_plt, w1(chi_plt, a_21).real, color=colors[3], label="Theory")
...
# axs[4].errorbar(chi[8],Re_1_1[8], fmt="o",color=colors[1],  ms=15, mfc="none")
...
axs[5].errorbar(chi[8], Im_1[8], fmt="o",color=colors[2],  ms=15, mfc="none")
# axs[5].errorbar(chi[8], Im_1[8], Im_1_err[8], fmt="k.", capsize=3,label="Data")
# axs[5].vlines(chi[8],-2,2, color=colors[1],  lw=1)

axs[4].legend(loc=9, ncol=1, handlelength=1)
axs[5].legend(loc=9, ncol=1, handlelength=1)
# axs[i].set_ylabel("Neutron rate (count / s)")
# axs[i].set_ylim([0,430])

plt.savefig("/home/aaa/Desktop/Fisica/PhD/2024/Grenoble 1st round/Paper/Images NatCom/Measurement example and wv.pdf", format="pdf",bbox_inches="tight")
plt.show()