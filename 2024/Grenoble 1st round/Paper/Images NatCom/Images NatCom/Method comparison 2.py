...
axs[0].set_title("Using Eq.(12)")
axs[1].set_title("Using Eq.(14)")
axs[0].set_ylabel("Real part of the weak value $w^\mathrm{R}_{1,+}$")
# axs[1].set_ylabel("Real part of the weak value $w^\mathrm{R}_{1,+}$")
# axs[0].set_ylabel("Weak value\nReal part $w^\mathrm{R}_{j,+}$")
# axs[1].set_ylabel("Weak value\nReal part $w^\mathrm{R}_{j,+}$")
...
for ax in axs:
    ...
    # ax.fill_between(chi_plt,1,4, color="#fceeee")#, label="Eigenvalue range")
...
axs[0].set_ylim([0,3])
axs[1].set_ylim([0,3])
axs[0].set_yticks([0,0.5,1, 2,3])
axs[1].set_yticks([0,0.5,1, 2,3])
axs[0].set_yticklabels([int(0),0.5,1, 2,3])

axs[1].set_ylim([0,3])
axs[1].set_yticks([0,0.5,1,2])
axs[1].set_yticklabels([int(0),0.5,1,2])

axs[0].plot(chi_plt, w1(chi_plt, a_21_unb).real, color=colors[3], lw=1.5)
axs[1].plot(chi_plt, w1(chi_plt, a_21_bal).real, color=colors[3], lw=1.5)
axs[0].errorbar(chi,Re_1_unb, Re_1_unb_err, fmt="k.", capsize=3, ms=4)
axs[1].errorbar(chi,Re_1_bal, Re_1_bal_err, fmt="k.", capsize=3, ms=4)

# axs[1].text(-np.pi, 1.1,"Eigenvalue bound", color=colors[1])
# axs[1].text(-np.pi, 1.1,"Eigenvalue range", color=colors[3])
# axs[0].legend()
    
# fig.savefig("/home/aaa/Desktop/Fisica/PhD/2024/Grenoble 1st round/Paper/Images NatCom/Results wv real different method 2.pdf", format="pdf",bbox_inches="tight")   
plt.show()