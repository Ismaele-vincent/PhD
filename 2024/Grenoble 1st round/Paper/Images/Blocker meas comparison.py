...
ax.errorbar(chi,Im, Im_err, fmt="b.", capsize=3)
...
gs = fig.add_gridspec(2, 1, hspace=0.00)
axs = [fig.add_subplot(gs[:, 0]),fig.add_subplot(gs[0, 0]),fig.add_subplot(gs[1, 0])]
axs[1].grid(True, ls="dotted")
axs[2].grid(True, ls="dotted")
axs[0].tick_params(axis="both", left=False, labelleft=False, bottom=False, labelbottom=False)
...
if lim:
    ...
    axs[1].plot(chi_plt, w1(chi_plt, a_21).real+0*(np.amax(w1(chi_plt, a_21).real)-np.amin(w1(chi_plt, a_21).real)), "--",color=colors[2], alpha=0.5, label="Theory")
    axs[1].errorbar(chi,Re_2+0.0*(np.amax(w1(chi_plt, a_21).real)-np.amin(w1(chi_plt, a_21).real)), Re_2_err, fmt=".", color=colors[2], capsize=4, label="Measurement")
...
axs[2].plot(chi_plt, w1(chi_plt, a_21).real, "--", color=colors[3], alpha=0.5, label="Theory")
axs[2].errorbar(chi,Re_1, Re_1_err, fmt=".", color=colors[3], capsize=4, label="Measurement")
axs[1].set_ylim([0.5,3])
axs[2].set_ylim([0.5,3])
axs[2].set_yticks(ticks=axs[2].get_yticks()[1:-1])
axs[1].set_yticks(ticks=axs[1].get_yticks()[1:-1])
axs[1].set_title("$w^\Re_{+,1}$")
# axs[2].set_ylabel("$w^\Re_{+,1}$")
axs[2].set_xlabel("$\chi$ [rad]")
axs[1].legend()
axs[2].legend()
plt.savefig("/home/aaa/Desktop/Fisica/PhD/2024/Grenoble 1st round/Paper/Images/Blocker measurement comparison.pdf", format="pdf",bbox_inches="tight")
plt.show()