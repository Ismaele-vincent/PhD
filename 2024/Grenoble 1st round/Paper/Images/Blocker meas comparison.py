...
axs[0].set_title("Using Eq.(19)", fontsize=10)
axs[1].set_title("Using Eq.(24)", fontsize=10)
axs[0].set_ylabel("$w^\Re_{+,1}$",fontsize=13)
axs[2].set_ylabel("$w^\Re_{+,1}$", fontsize=13)
axs[1].set_ylabel("$\mathdefault{a_2/a_1\\approx 0.59}$", rotation=270, labelpad=15)
axs[3].set_ylabel("$\mathdefault{a_2/a_1\\approx 1}$", rotation=270, labelpad=15)
...
for inf_file_name in inf_file_names:
    ...
    s=np.sign(sa)*abs(sa)**0.5
    Re_1=(1+s)/2
    Re_1_err=((data_ifg_matrix_err[2]/data_ifg_matrix[0])**2+(data_ifg_matrix[2]*data_ifg_matrix_err[0]/data_ifg_matrix[0]**2)**2+64*Im_1**2*Im_1_err**2)**0.5/abs(s)/4
