...
fig = plt.figure(figsize=(9,7*2/3), dpi=300)
fig.text(0.05, 0.9, "(a)", fontweight='bold')
fig.text(0.5, 0.9, "(b)", fontweight='bold')
gs_b = fig.add_gridspec(1,2 , wspace=0.4)
gs= GS.GridSpecFromSubplotSpec(1,2, hspace=0.1, wspace=0, subplot_spec=gs_b[0,0])
gs_1= GS.GridSpecFromSubplotSpec(1,2, hspace=0.1, wspace=0, subplot_spec=gs_b[0,1])
# gs = fig.add_gridspec(1,2, wspace=0.0)
# gs_1 = fig.add_gridspec(1,2, wspace=0.0)
axs = [fig.add_subplot(gs[0, 0]),fig.add_subplot(gs[0, 1]),fig.add_subplot(gs_1[0, 0]),fig.add_subplot(gs_1[0, 1])]
...
axs[1].plot([1.085,1.085], [0,1], transform=axs[1].transAxes, lw=1.5, color="#cc00cc",clip_on=False)
axs[1].plot([1.085,1.085], [0.2,0.8], transform=axs[1].transAxes, lw=3., color='w', clip_on=False)
axs[1].text(1.085, 0.5, "Unbalanced - Using Eq.(14)",  color="#cc00cc", ha="center", va="center", rotation=-90, transform=axs[1].transAxes)
axs[3].plot([1.085,1.085], [0,1], transform=axs[3].transAxes, lw=1.5, color="#e67e22",clip_on=False)
axs[3].plot([1.085,1.085], [0.23,0.77], transform=axs[3].transAxes, lw=3., color='w', clip_on=False)
axs[3].text(1.085, 0.5, "Balanced - Using Eq.(12)", color="#e67e22", ha="center", va="center", rotation=-90, transform=axs[3].transAxes)
...
for ax in axs:
    ...
    ax.plot(chi_plt, chi_plt*0+0.5, "--", color=colors[2], lw=0.5)#, label="Eigenvalue bound")
...
# axs[0].legend()
fig.savefig("/home/aaa/Desktop/Fisica/PhD/2024/Grenoble 1st round/Paper/Review npj QI/Figure6.pdf", format="pdf",bbox_inches="tight")   
plt.show()