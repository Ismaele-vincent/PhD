...
axs_re = [fig.add_subplot(gs[:4, 0]), fig.add_subplot(gs[:4, 1]),fig.add_subplot(gs[4:, 0]),fig.add_subplot(gs[4:, 1])]

# gs = fig.add_gridspec(2,2 , height_ratios=(2,1), hspace=0.03, wspace=0.0)
# axs_re = [fig.add_subplot(gs[0, 0]),fig.add_subplot(gs[0, 1]),fig.add_subplot(gs[1, 0]),fig.add_subplot(gs[1, 1])]
# axs_re[0].set_title("$w_{1,+}$", fontsize=13)
# axs_re[1].set_title("$w_{2,+}$", fontsize=13)
...
axs_re[1].plot([1.07,1.07], [0,1], transform=axs_re[1].transAxes, lw=1.5, color="#cc00cc",clip_on=False)
axs_re[1].plot([1.07,1.07], [0.37,0.63], transform=axs_re[1].transAxes, lw=3., color='w', clip_on=False)
axs_re[1].text(1.07, 0.5, "Unbalanced", color="#cc00cc", ha="center", va="center", rotation=-90, transform=axs_re[1].transAxes)
axs_re[3].plot([1.07,1.07], [0,1], transform=axs_re[3].transAxes, lw=1.5, color="#e67e22",clip_on=False)
axs_re[3].plot([1.07,1.07], [0.30,0.7], transform=axs_re[3].transAxes, lw=3., color='w', clip_on=False)
axs_re[3].text(1.07, 0.5, "Balanced", color="#e67e22", ha="center", va="center", rotation=-90, transform=axs_re[3].transAxes)


# axs_re[1].text(1.1, 0.5, "Unbalanced", ha="center", va="center", transform=axs_re[1].transAxes)
# axs_re[2].text(0.25, 0.02, "Balanced", ha="center", va="center", transform=ax_b.transAxes)
# axs_re[3].text(0.75, 0.02, "Balanced", ha="center", va="center", transform=ax_b.transAxes)
...
# axs_re[1].text(-np.pi, 1.1,"Eigenvalue bound", color=colors[1])
# axs_re[1].text(-np.pi, 1.1,"Eigenvalue range", color=colors[3])
# axs_re[0].legend()
    
# fig.savefig("/home/aaa/Desktop/Fisica/PhD/2024/Grenoble 1st round/Paper/Review npj QI/Results wv real.pdf", format="pdf",bbox_inches="tight")   
plt.show()