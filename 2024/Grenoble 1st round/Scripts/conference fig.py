...
# -*- coding: utf-8 -*-
"""
Created on Mon May  6 15:28:35 2024
...
fig = plt.figure(figsize=(8,6))
ax = fig.add_subplot(111)
ax.set_title("Path weak value exctracted from interferogram")
ax.errorbar(chi,Im, Im_err, fmt="k.", capsize=3, label="$\Im(w_{1,+})$ data")
ax.plot(chi_plt, w1(chi_plt, a_21).imag, "k--", alpha=0.5, label="$\Im(w_{1,+})$ theory" )
...
Re=(1+s)/2
Re_err=(data_ifg_matrix_err[2]**2/data_ifg_matrix[0]**2+data_ifg_matrix[2]**2*data_ifg_matrix_err[0]**2/data_ifg_matrix[0]**4+64*Im**2*Im_err**2)**0.5/s
...
ax.plot(chi_plt, w1(chi_plt, a_21).real, "g--", alpha=0.5, label="$\Re(w_{1,+})$ theory")
ax.errorbar(chi,Re, Re_err, fmt="g.", capsize=4, label="$\Re(w_{1,+})$ data")
# ax.set_ylim([-0.8,2.2])
ax.legend(ncol=1)
plt.savefig("/home/aaa/Desktop/Fisica/PhD/2024/Grenoble 1st round/Report/Images/Conference_plot.png", format="png",dpi=200, bbox_inches="tight")
# plt.show()