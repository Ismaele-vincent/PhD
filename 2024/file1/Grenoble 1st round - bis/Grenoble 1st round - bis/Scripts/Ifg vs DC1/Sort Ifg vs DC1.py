...
for root1, dirs1, files1 in os.walk(sc_fold_path, topdown=False):
    for name1 in files1:
        if ("ifg_vs_DC" in name1) and (".inf" in name1):
            if (name1[:-4] in bad_apples):
                ...
                sorted_fold_path="C:/Users/S18/Desktop/Grenoble-2024 Ismaele/2024/Grenoble 1st round - bis/exp_CRG-3126/Sorted data/Ifg vs DC1/"+inf_file_name
                ...
                # # print(dat_files)
                # Amp=np.array([range(len(dat_files[:,0])), dat_files[:,-2].astype(float)])
                # print(Amp)
                ...
                #             Amp_aux=i*0.5#dat_files[:,2][dat_files[:,0]==name].astype(float) #phashifter position
                #             # print(curr)
                #             cleantxt =np.hstack((txt,np.ones((len(txt[:,0]),1))*Amp_aux))
                #             # cleantxt =txt
                #             with open(cleandata+"/ifg_vs_Amp2_"+str("%02d" % (Amp[0][dat_files[:,0]==name],))+".txt", 'w') as f:
