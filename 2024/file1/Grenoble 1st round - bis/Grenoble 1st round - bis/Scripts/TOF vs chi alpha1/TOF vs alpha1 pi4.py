...
rad=np.pi/180
a_21=1
a_1=0.443**0.5#1/2**0.5
a_2=0.556**0.5#1/2**0.5
a_21=a_2/a_1
...
def fit_cos(x, A, B, C, D):
    return A*(1/2+a_1*a_2*B*jv(0,alpha_1)*np.cos(C*x-D))
A_aus=1

inf_file_name="TOF_vs_chi_alpha1_22pt_pi4_2kHz_1200s_03Apr0436"
...
p_ifg,cov_ifg=fit(fit_cos, ps_pos, ps_data, p0=P0,  bounds=B0)
err_ifg=np.diag(cov_ifg)**0.5
Co = p_ifg[1]
A=p_ifg[0]*(1-Co)/2
A_err= (((1-Co)/2*err_ifg[0])**2+(p_ifg[0]/2*err_ifg[1])**2)**0.5
A_aus=p_ifg[0]/len(time)
w_ps=p_ifg[-2]
chi_0=p_ifg[-1]
chi_0_err=err_ifg[-1]
...
# def fit_Im(t, xi_1, Im_1, Re_1):
#     return A_aus*((1-Co)/2+Co*ps_data_aus*(1+2*Re_1*(1-np.cos(alpha_1*np.sin(2*np.pi*1e-3*f_1*t+xi_1)))+2*Im_1*np.sin(alpha_1*np.sin(2*np.pi*1e-3*f_1*t+xi_1))))
...
for i in range(len(ps_pos)):
    ...
    ps_data_aus=fit_cos(chi[i], 1,*p_ifg[1:])
    chi_aus=chi[i]
    
    P0=[np.cos(chi[i]/2)**2, w1(chi[i]).imag, abs(w1(chi[i]))-w1(chi[i]).real-1000, 0.5]
    # print(P0)
    ...
    # P0=[0.5, w1(chi[i]).imag, abs(w1(chi[i]))-w1(chi[i]).real-1000]
    # # print(P0)
    # B0=([-2*np.pi, w1(chi[i]).imag-1000,abs(w1(chi[i]))-w1(chi[i]).real-1000],[2*np.pi, w1(chi[i]).imag+1000, abs(w1(chi[i]))-w1(chi[i]).real+1000])
    ...
    Re_data_err_1_fit[i]=err_P#4/(Re_data_1_fit[i]-0.5)*err_P
