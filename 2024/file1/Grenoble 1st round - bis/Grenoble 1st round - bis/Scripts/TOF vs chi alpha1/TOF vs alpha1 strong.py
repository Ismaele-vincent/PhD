...
B0=([100,0,0.01,-10],[np.amax(ps_data)+100000,np.amax(ps_data)+10000,5, 10])
...
for i in range(len(ps_pos)):
    ...
    chi_aus=chi[i]
    P0=[np.cos(chi[i]/2)**2, w1(chi[i]).imag, abs(w1(chi[i]))-w1(chi[i]).real-1000, -2.5]
    # print(P0)
    ...
    Re_data_1_fit[i]=0.5*(1-np.sqrt(1+4*P))
    Re_data_err_1_fit[i]=4/np.sqrt(1+4*P)*err_P
