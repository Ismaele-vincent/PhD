...
for root, dirs, files in os.walk(sorted_fold_path, topdown=False):
    ...
    for name in files[:]:
        if (name not in bad_apples):
            ...
            data_count=tot_data[2]#/(tot_data[6]*5e-10)
            if ("60s_In1" in name):
...
# counts_array+=A/3*(2*(1-C_12)*a_1*a_2*np.cos(chi_1+chi_1_0-chi_2-chi_2_0) + 2*(1-C_13)*a_1*a_3*np.cos(chi_1+chi_1_0-chi_3-chi_3_0) + 2*(1-C_23)*a_2*a_3*np.cos(chi_2+chi_2_0-chi_3-chi_3_0))
