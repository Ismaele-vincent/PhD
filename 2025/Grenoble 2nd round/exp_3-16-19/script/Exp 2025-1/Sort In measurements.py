...
bad_apples=["count_60s_In1_+1+1+1_23Oct1915.dat","count_60s_In1_+1-i-1_23Oct1927.dat","count_60s_In1_+exp(-i60)+1+exp(i120)_23Oct1939.dat", "count_60s_In3_+1+1+1_23Oct1953.dat" #No In measurement
            ]
...
for root1, dirs1, files1 in os.walk(sc_fold_path, topdown=False):
    for name1 in files1:
        # if ("count_60s_" in name1) and (".dat" in name1) and ("23Oct19" in name1):
        #     print(name1)
        
        if ("count_60s_" in name1) and ("+1+1+1" in name1) and (".dat" in name1):
            ...
            if (name1 not in bad_apples):
                ...
                name1=name1[:10]+"No_In"+name1[13:]
                print(name1)
                with open(cleandata+"/"+name1[:-4]+".txt", 'w') as f:
                        np.savetxt(f, cleantxt_ifg, delimiter="\t", header= "ps_pos exposure_time(s) O-Beam H-Beam Monitor AUX-Beam time(s) O+H+AUX encod1 encod2")#, fmt='%i %f %.1f %i %i %i %i %i %i %f %f')
        
        if ("count_60s_" in name1) and ("+1-i-1" in name1) and (".dat" in name1):
            # inf_file_path=os.path.join(root1, name1)
            # print(name1)
            # dat_file=np.loadtxt(inf_file_path, encoding='windows-1252', usecols=0, comments="*",delimiter="\t", dtype=str)
            ...
            if (name1 not in bad_apples):
                ...
                with open(cleandata+"/"+name1[:-4]+".txt", 'w') as f:
                        np.savetxt(f, cleantxt_ifg, delimiter="\t", header= "ps_pos exposure_time(s) O-Beam H-Beam Monitor AUX-Beam time(s) O+H+AUX encod1 encod2")#, fmt='%i %f %.1f %i %i %i %i %i %i %f %f')
        if ("count_60s_" in name1) and ("+exp(-i60)+1+exp(i120)" in name1) and (".dat" in name1):
            # inf_file_path=os.path.join(root1, name1)
            # print(name1)
            # dat_file=np.loadtxt(inf_file_path, encoding='windows-1252', usecols=0, comments="*",delimiter="\t", dtype=str)
