...
"""
"ifg_wv2_psi_+1+exp(-i60)+exp(i120)_no_fit_22Oct1812"
"""
C_12=0.74
C_13=0.66
C_23=0.62
...
good_apples=["ifg_wv2_psi_+1+exp(-i60)+exp(i120)_no_fit_21Oct1356"#bit noisy
            "ifg_wv2_psi_+1+exp(-i60)+exp(i120)_no_fit_22Oct1812" #soso
            "ifg_wv2_psi_+1+exp(-i60)+exp(i120)_no_fit_24Oct2156" #bad
             ]

inf_file_names=["ifg_wv2_psi_+1+exp(-i60)+exp(i120)_no_fit_22Oct1812" #soso
...
def fit_C(x, C_13, B, D, E):
    return A/3*(1-C_13*a_1*a_3+2*B*np.cos(D*x-E))
...
ps_pos=tot_data[:,0]
P0=[(np.amax(data_O)+np.amin(data_O))/2, 0.7, 6, 4]
