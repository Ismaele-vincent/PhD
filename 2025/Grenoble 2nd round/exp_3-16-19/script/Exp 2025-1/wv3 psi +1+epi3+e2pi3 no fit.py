...
"""
"ifg_wv3_psi_+1+1+1_no_fit_22Oct1212"
"""
C_12=0.72 #86
C_13=0.67 
C_23=0.58
...
inf_file_names=[
                "ifg_wv3_psi_+1+exp(-i60)+exp(i120)_no_fit_22Oct1852" #best - phase drift
                # "ifg_wv3_psi_+1+exp(-i60)+exp(i120)_no_fit_24Oct2236" #bit noisy
]
...
ps_pos=tot_data[:,0]
P0=[(np.amax(data_O)+np.amin(data_O))/2, 0.7, 6.1, 1.4]
B0=([np.amin(data_O)/2,0,5.01,-2*np.pi],[np.amax(data_O)*3,np.amax(data_O)*2,7, 2*np.pi])
...
B0_C=([0.3,0,0,0.01],[1,1,7, 2*np.pi])
