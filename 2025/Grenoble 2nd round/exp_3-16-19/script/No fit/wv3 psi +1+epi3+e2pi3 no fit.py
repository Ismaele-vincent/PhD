...
for inf_file_name in inf_file_names:
        ...
        for root, dirs, files in os.walk(cleandata, topdown=False):
            ...
            ps_pos=tot_data[:,0]
            P0=[(np.amax(data_ifg)+np.amin(data_ifg))/2, (np.amax(data_ifg)-np.amin(data_ifg))/2, 6, 2.4]
        ...
        print(A/3*(1+2*(C_23*a_2*a_3*np.cos(chi_2_0+chi_3_0))), np.average(data_ifg_matrix[0]))
        ...
        chi_3_0_aux=chi_3_0 
        for i in [1,0,2,3]:
            chi_3_0=chi_3_0_aux-np.pi/2+k*np.pi/2
            ...
            k+=1
        chi_3_0=chi_3_0_aux
        P3=int_data[2]*time_ifg/time_int
        P3_err=P3**0.5
        # P2_corr=C_id*P2
        ...
        fig = plt.figure(figsize=(8,6))
        ax = fig.add_subplot(111)
        ax.errorbar(chi_3, data_ifg_matrix[0],yerr=data_ifg_matrix[0]**0.5,fmt="ko",capsize=5, ms=3)
        ax.plot(chi_3_plt, I_corr(A, chi_1, chi_2, chi_3_plt))
        
        
        Re_3=P3/data_ifg_matrix[0] + 1/4 - data_ifg_matrix[3]/(data_ifg_matrix[0]*4)
        Re_3_err=(P3_err**2+(data_ifg_matrix_err[2]/4)**2+(Re_3-1/4)**2*data_ifg_matrix_err[0]**2)**0.5/abs(data_ifg_matrix[0])
        
        Im_3=(data_ifg_matrix[1]-data_ifg_matrix[2])/data_ifg_matrix[0]/4
        Im_3_err=(data_ifg_matrix_err[1]**2+data_ifg_matrix_err[3]**2+(4*Im_3)**2*data_ifg_matrix_err[0]**2)**0.5/(4*abs(data_ifg_matrix[0]))
