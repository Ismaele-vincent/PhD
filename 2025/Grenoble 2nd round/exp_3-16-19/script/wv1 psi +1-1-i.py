...
bad_apples=["ifg_wv1_psi_+1-1-i_16Oct0245", #fit didn't reach 180 shift (-405)
            "ifg_wv1_psi_+1-1-i_17Oct0442", #wrong initial state
            "ifg_wv1_psi_+1-1-i_20Oct0859" #wrong initial state
            ]
good_apples=["ifg_wv1_psi_+1-1-i_12Oct1018",
             "ifg_wv1_psi_+1-1-i_14Oct1640", #15 s per point, slight phase drift
             "ifg_wv1_psi_+1-1-i_15Oct0606", #good
    ]
inf_file_names=["ifg_wv1_psi_+1-1-i_15Oct0606"
...
for inf_file_name in inf_file_names:
        ...
        for root, dirs, files in os.walk(cleandata, topdown=False):
            ...
            for name in files[:-1]:
                ...
                ps_pos=tot_data[:,0]
                P0=[(np.amax(data_ifg)+np.amin(data_ifg))/2, (np.amax(data_ifg)-np.amin(data_ifg))/2, 6, -7.6]
                B0=([np.amin(data_ifg),0,0.01,-5*np.pi],[np.amax(data_ifg)*2,np.amax(data_ifg)*2,7, 1*np.pi])
                ...
                # print(p[3])
                if i==0:
                    chi=(ps_pos-ps_pos[0])*p[2]-2*np.pi
                i+=1
        ...
        # data_ifg_matrix+=A/9*(2*(1-C_23)+2*(1-(C_12+C_13))*np.cos(chi))
        P1=int_data[1]*time_ifg/time_int
        P1_corr=P1*C_id#*P3
        ...
        Re_1=P1_corr/data_ifg_matrix[0] + 1/4 - data_ifg_matrix[3]/(data_ifg_matrix[0]*4)
        Re_1_err=(P1_corr_err**2+(data_ifg_matrix_err[2]/4)**2+(Re_1-1/4)**2*data_ifg_matrix_err[0]**2)**0.5/abs(data_ifg_matrix[0])
        
        Im_1=(data_ifg_matrix[1]-data_ifg_matrix[2])/data_ifg_matrix[0]/4
        ...
        fig.suptitle("$|\\psi_{in}>=(|1>-|2>-i|3>)/\\sqrt{3}$")
        ...
        axs[0].errorbar(chi_plt, w1(chi_plt,0,0).real, color=colors[3], alpha=0.8)
        
        axs[1].errorbar(chi,Im_1, Im_1_err, fmt="k.", capsize=3)
        axs[1].plot(chi_plt, w1(chi_plt,0,0).imag, color=colors[3], alpha=0.8 )
        
        # axs[2].errorbar(chi,Re_1, Re_1_err, fmt="k.", capsize=3)
        axs[2].errorbar(chi_plt, w2(chi_plt,0,0).real, color=colors[3], alpha=0.8)
        
        # axs[3].errorbar(chi,Im_1, Im_1_err, fmt="k.", capsize=3)
        axs[3].plot(chi_plt, w2(chi_plt,0,0).imag, color=colors[3], alpha=0.8 )
        
        # axs[4].errorbar(chi,Re_1, Re_1_err, fmt="k.", capsize=3)
        axs[4].errorbar(chi_plt, w3(chi_plt,0,0).real, color=colors[3], alpha=0.8)
        
        # axs[5].errorbar(chi,Im_1, Im_1_err, fmt="k.", capsize=3)
        axs[5].plot(chi_plt, w3(chi_plt,0,0).imag, color=colors[3], alpha=0.8 )
