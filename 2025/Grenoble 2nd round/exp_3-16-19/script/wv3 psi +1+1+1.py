...
for inf_file_name in inf_file_names:
        ...
        for root, dirs, files in os.walk(cleandata, topdown=False):
            ...
            for name in files[:-1]:
                ...
                ps_pos=tot_data[:,0]
                P0=[(np.amax(data_ifg)+np.amin(data_ifg))/2, (np.amax(data_ifg)-np.amin(data_ifg))/2, 6, -6.1]
                B0=([np.amin(data_ifg),0,0.01,-3*np.pi],[np.amax(data_ifg)*2,np.amax(data_ifg)*2,7, np.pi])
        ...
        P3_corr=P3*C_id#*P3
        P3_corr_err=(C_id**2*P3+P3**2*C_id_err**2)**0.5
        # P2_corr=C_id*P2
        # P2_corr_err=(C_id**2*P2+P2**2*C_id_err**2)**0.5
        
        chi_plt=np.linspace(chi[0], chi[-1], 1000)
        
        Re_3=P3_corr/data_ifg_matrix[0] + 1/4 - data_ifg_matrix[3]/(data_ifg_matrix[0]*4)
        Re_3_err=(P3_corr_err**2+(data_ifg_matrix_err[2]/4)**2+(Re_3-1/4)**2*data_ifg_matrix_err[0]**2)**0.5/abs(data_ifg_matrix[0])
        ...
        axs[0].errorbar(chi_plt, w1(0,0,chi_plt).real, color=colors[3], alpha=0.8)
        
        # axs[1].errorbar(chi,Im_1, Im_1_err, fmt="k.", capsize=3)
        axs[1].plot(chi_plt, w1(0,0,chi_plt).imag, color=colors[3], alpha=0.8 )
        
        # axs[2].errorbar(chi,Re_3, Re_3_err, fmt="k.", capsize=3)
        axs[2].errorbar(chi_plt, w2(0,0,chi_plt).real, color=colors[3], alpha=0.8)
        
        # axs[3].errorbar(chi,Im_3, Im_3_err, fmt="k.", capsize=3)
        axs[3].plot(chi_plt, w2(0,0,chi_plt).imag, color=colors[3], alpha=0.8 )
        
        axs[4].errorbar(chi,Re_3, Re_3_err, fmt="k.", capsize=3)
        axs[4].errorbar(chi_plt, w3(0,0,chi_plt).real, color=colors[3], alpha=0.8)
        
        axs[5].errorbar(chi,Im_3, Im_3_err, fmt="k.", capsize=3)
        axs[5].plot(chi_plt, w3(0,0,chi_plt).imag, color=colors[3], alpha=0.8 )
        ...
        # axs[0].errorbar(chi,Re_3_1, Re_3_1_err, fmt="k.", capsize=3)
